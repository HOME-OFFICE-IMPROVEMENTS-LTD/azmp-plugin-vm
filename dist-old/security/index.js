...
exports.trustedlaunch = exports.encryption = exports.securityTemplates = void 0;
exports.createSecurityHelpers = createSecurityHelpers;
const encryption = __importStar(require("./encryption"));
exports.encryption = encryption;
const trustedlaunch = __importStar(require("./trustedlaunch"));
exports.trustedlaunch = trustedlaunch;
/**
 * Security templates catalog
 */
exports.securityTemplates = [
    {
...
function createSecurityHelpers() {
    const helpers = {};
    // Encryption Helpers
    helpers["security:encryption.ade"] = function (platform, options) {
        const config = options.hash || {};
        return JSON.stringify(encryption.azureDiskEncryption(platform, config), null, 2);
    };
    helpers["security:encryption.sse"] = function (options) {
        const config = options.hash || {};
    ...
    helpers["security:encryption.cmk"] = function (keyVaultId, keyUrl) {
        return JSON.stringify(encryption.customerManagedKey(keyVaultId, keyUrl), null, 2);
    };
    helpers["security:encryption.platform"] = function () {
        return JSON.stringify(encryption.platformManagedEncryption(), null, 2);
    };
    helpers["security:encryption.double"] = function (options) {
        const config = options.hash || {};
        return JSON.stringify(encryption.doubleEncryption(config), null, 2);
    };
    helpers["security:encryption.recommendations"] = function (compliance) {
        return encryption.getEncryptionRecommendations(compliance);
    };
    // Trusted Launch Helpers
    helpers["security:trustedLaunch.secureBoot"] = function (enabled = true) {
        return JSON.stringify(trustedlaunch.secureBoot(enabled), null, 2);
    };
    helpers["security:trustedLaunch.vTpm"] = function (enabled = true) {
        return JSON.stringify(trustedlaunch.vTpm(enabled), null, 2);
    };
    helpers["security:trustedLaunch.bootIntegrity"] = function (enabled = true) {
        return JSON.stringify(trustedlaunch.bootIntegrityMonitoring(enabled), null, 2);
    };
    helpers["security:trustedLaunch.guestAttestation"] = function (platform) {
        return JSON.stringify(trustedlaunch.guestAttestationExtension(platform), null, 2);
    };
    helpers["security:trustedLaunch.measuredBoot"] = function () {
        return JSON.stringify(trustedlaunch.measuredBoot(), null, 2);
    };
    helpers["security:trustedLaunch.config"] = function (level, platform) {
        return JSON.stringify(trustedlaunch.createTrustedLaunchConfig(level, platform), null, 2);
    };
    helpers["security:trustedLaunch.recommendations"] = function (compliance) {
        return trustedlaunch.getTrustedLaunchRecommendations(compliance);
    };
    ...
    helpers["security:template.get"] = function (templateName) {
        return exports.securityTemplates.find((t) => t.name === templateName);
    };
    helpers["security:template.list"] = function (compliance) {
        if (compliance) {
            return exports.securityTemplates.filter((t) => t.compliance.some((c) => c.toLowerCase().includes(compliance.toLowerCase())));
        }
        return exports.securityTemplates;
    };
    helpers["security:template.listCompliance"] = function () {
        const complianceSet = new Set();
        exports.securityTemplates.forEach((t) => t.compliance.forEach((c) => complianceSet.add(c)));
        return Array.from(complianceSet).sort();
    };
    // Compliance Helpers
    helpers["security:compliance.getRequirements"] = function (framework) {
        const encRec = encryption.getEncryptionRecommendations(framework);
        const tlRec = trustedlaunch.getTrustedLaunchRecommendations(framework);
        return {
            framework,
            encryption: encRec,
            trustedLaunch: tlRec,
            templates: exports.securityTemplates.filter((t) => t.compliance.some((c) => c.toLowerCase().includes(framework.toLowerCase()))),
        };
    };
    ...
    helpers["security:count.templates"] = function () {
        return exports.securityTemplates.length;
    };
...
__exportStar(require("./trustedlaunch"), exports);
