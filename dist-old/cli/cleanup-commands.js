...
exports.registerCleanupCommands = registerCleanupCommands;
const child_process_1 = require("child_process");
const path = __importStar(require("path"));
const fs = __importStar(require("fs/promises"));
const readline = __importStar(require("readline"));
const approval_manager_1 = require("../utils/approval-manager");
/**
 ...
 */
async function validatePowerShell() {
    return new Promise((resolve, reject) => {
        const child = (0, child_process_1.spawn)("pwsh", ["--version"]);
        child.on("close", (code) => {
            if (code === 0) {
                resolve();
            }
            else {
                reject(new Error("PowerShell 7+ (pwsh) is required but not found. " +
 ...
 */
async function executePowerShellScript(command, args, context) {
    return new Promise((resolve) => {
 ...
 */
async function executeVaultCleanup(options, context) {
    ...
    if (!options.dryRun && !options.force) {
        if (!options.confirm) {
            throw new Error("Destructive operations require explicit confirmation.\n" +
    ...
    if (options.force && !options.dryRun && enforceApproval) {
        ...
        const approvalManager = new approval_manager_1.ApprovalManager();
        const approval = approvalManager.findApprovalByVault(options.vaultName, options.resourceGroup);
        if (!approval) {
            throw new Error("Approval required but not found.\n" +
        ...
        if (now >= expiresAt) {
            throw new Error(`Approval exists but has EXPIRED (expired at ${approval.expiresAt}).\n` +
    ...
    if (options.json) {
        ...
        if (result.success) {
            ...
            try {
                const dryRunResult = JSON.parse(result.stdout);
 ...
 */
async function executeVaultApprove(dryRunFile, options, context) {
    context.logger.info("📝 Approving dry-run result...");
    // Parse dry-run file
    let dryRunResult;
    try {
        dryRunResult = approval_manager_1.ApprovalManager.parseDryRunFile(dryRunFile);
 ...
 */
async function executeVaultCheck(options, context) {
    if (!options.vaultName) {
        throw new Error("Vault name is required. Use --vault-name option.");
    }
    if (!options.resourceGroup) {
 ...
 */
function registerCleanupCommands(parentCommand, options) {
    ...
    cleanupCommand
        ...
        .action(async (dryRunFile, cmdOptions) => {
        try {
