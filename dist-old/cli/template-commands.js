...
const prune_template_1 = require("../utils/prune-template");
/**
 ...
 */
function registerTemplateCommands(parentCommand, options) {
    ...
    templateCommand
        ...
        .option("-o, --output <dir>", "Output directory for generated templates", "./output")
        ...
        try {
            ...
            Handlebars.registerHelper('includesParam', function (paramName, options) {
            ...
            if (templates.main) {
                ...
                if (await pathExists(mainTemplatePath)) {
                    ...
                    const prunedResult = (0, prune_template_1.pruneTemplate)(rawOutput, context.logger);
            ...
            if (templates.ui) {
                const uiTemplatePath = path.join(templatesDir, "createUiDefinition.json.hbs");
            ...
            if (templates.view) {
                const viewTemplatePath = path.join(templatesDir, "viewDefinition.json.hbs");
            ...
            context.logger.info(`  2. Run validation: azmp vm template validate ${outputDir}`);
            context.logger.info(`  3. Test deployment: azmp vm template test ${outputDir}`);
        }
        catch (error) {
            context.logger.error(`Template generation failed: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
    });
    // ========================================
    // template validate
    ...
    templateCommand
        ...
        .action(async (templatePath, cmdOptions) => {
        try {
                ...
                try {
                    ...
                    if (fileName === "mainTemplate.json") {
                        ...
                        if (!json.resources || json.resources.length === 0) {
                            ...
                            context.logger.info(`  ✓ Found ${json.resources.length} resource(s)`);
                        }
                        if (json.outputs) {
                            context.logger.info(`  ✓ Found ${Object.keys(json.outputs).length} output(s)`);
                        }
                    }
                        ...
                        if (!json.parameters || !json.parameters.steps) {
                            ...
                            context.logger.info(`  ✓ Found ${json.parameters.steps.length} wizard step(s)`);
                        }
                    ...
                    context.logger.error(`  ✗ JSON parsing error: ${parseError instanceof Error ? parseError.message : "Unknown error"}`);
                    allValid = false;
                    totalErrors++;
                }
            }
            ...
            if (cmdOptions.parameters && files.length > 1) {
                ...
                const mainTemplatePath = files.find((f) => f.endsWith("mainTemplate.json"));
                const uiDefinitionPath = files.find((f) => f.endsWith("createUiDefinition.json"));
                if (mainTemplatePath && uiDefinitionPath) {
                    try {
                        ...
                        const uiOutputs = Object.keys(uiDefinition.parameters?.outputs || {});
                        // Check if all UI outputs map to main template parameters
                        const unmappedOutputs = uiOutputs.filter((output) => !mainParams.includes(output));
                        if (unmappedOutputs.length > 0) {
                            context.logger.warn(`  ⚠ ${unmappedOutputs.length} UI output(s) not found in mainTemplate parameters:`);
                            ...
                            context.logger.info("  ✓ All UI outputs map to mainTemplate parameters");
                        }
                    }
                    catch (error) {
                        context.logger.error(`  ✗ Parameter consistency check failed: ${error instanceof Error ? error.message : "Unknown error"}`);
                        totalErrors++;
                    }
                }
            }
            ...
            if (allValid && totalErrors === 0) {
                context.logger.info(`✅ Validation passed for ${files.length} file(s)`);
                if (totalWarnings > 0) {
                    context.logger.info(`⚠  ${totalWarnings} warning(s) found`);
                }
            }
            else {
                context.logger.error(`❌ Validation failed with ${totalErrors} error(s) and ${totalWarnings} warning(s)`);
            }
            if (!cmdOptions.schemaOnly) {
                context.logger.info("\n💡 Tip: For comprehensive validation, use ARM-TTK:");
                context.logger.info("   npm run install-arm-ttk && azmp validate " + resolvedPath);
            }
        }
        catch (error) {
            context.logger.error(`Validation failed: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
    });
    // ========================================
    // template test
        ...
        try {
            ...
            if (cmdOptions.parameters) {
                ...
                if (!(await pathExists(parametersPath))) {
                    context.logger.error(`Parameters file not found: ${parametersPath}`);
                    return;
                }
            ...
            let validParams = 0;
            for (const [paramName, paramDef] of Object.entries(templateParams)) {
                ...
                if (!hasDefault && !isProvided) {
                    context.logger.error(`  ✗ Missing required parameter: ${paramName}`);
                    missingRequired++;
                }
                else {
                    validParams++;
            ...
            context.logger.info(`✓ All ${validParams} required parameters satisfied`);
            // Dry run analysis
            if (cmdOptions.dryRun) {
                ...
                context.logger.info("   2. Create resource group: az group create -n myRG -l eastus");
                context.logger.info(`   3. Deploy: az deployment group create -g myRG --template-file ${mainTemplatePath} --parameters @parameters.json`);
            }
            else {
            ...
            context.logger.error(`Template test failed: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
    });
    // ========================================
    ...
    templateCommand
        ...
        .option("-l, --location <location>", "Azure region (e.g., eastus)", "eastus")
        .action(async (cmdOptions) => {
        ...
        context.logger.info("  3. Generated ARM templates (mainTemplate.json, createUiDefinition.json, viewDefinition.json)");
        context.logger.info("\n🔧 Deployment Options:");
        context.logger.info("\nOption 1: Azure Portal (Recommended for testing)");
        context.logger.info("  1. Navigate to Azure Portal: https://portal.azure.com");
        ...
        if (cmdOptions.resourceGroup) {
            context.logger.info(`  # Deploy to resource group: ${cmdOptions.resourceGroup}`);
            context.logger.info(`  az deployment group create \\`);
            context.logger.info(`    --resource-group ${cmdOptions.resourceGroup} \\`);
        }
        else {
            context.logger.info("  # Create resource group");
            context.logger.info(`  az group create --name myResourceGroup --location ${cmdOptions.location}`);
            context.logger.info("");
        ...
        context.logger.info("  • ARM Template docs: https://docs.microsoft.com/azure/azure-resource-manager/templates/");
        context.logger.info("  • Partner Center: https://partner.microsoft.com/dashboard/marketplace-offers/overview");
        context.logger.info("  • Marketplace docs: https://docs.microsoft.com/azure/marketplace/");
        context.logger.info("\n💡 Testing Workflow:");
        ...
        context.logger.info("  3. Test: azmp vm template test ./output --dry-run");
