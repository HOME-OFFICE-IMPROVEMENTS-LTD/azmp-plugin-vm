...
const monitoring_1 = require("../../azure/monitoring");
// ============================================================================
// Command Implementation
// ============================================================================
const configureMonitoringCommand = new commander_1.Command('configure-monitoring')
...
// Command Handler
// ============================================================================
async function handleConfigureMonitoring(options) {
    ...
    if (options.preset) {
        ...
        const preset = monitoring_1.MonitoringManager.PRESETS[presetName];
        if (!preset) {
        ...
        manager = new monitoring_1.MonitoringManager(config);
    }
    else {
...
// Helper Functions
// ============================================================================
function createFromCustomOptions(options) {
    const config = {
        vmName: options.vmName,
        alertRules: [],
    ...
    const manager = new monitoring_1.MonitoringManager(config);
...
function addCpuAlert(manager, vmName, threshold, severity, email) {
    const rule = {
        name: 'high-cpu',
        description: 'Alert when CPU usage is high',
        metricName: monitoring_1.MetricType.CPU,
        operator: monitoring_1.AlertOperator.GreaterThan,
        threshold,
        severity,
        timeAggregation: monitoring_1.TimeAggregation.Average,
        windowSize: 'PT5M',
...
function addMemoryAlert(manager, vmName, threshold, severity, email) {
    const rule = {
        name: 'low-memory',
        description: 'Alert when available memory is low',
        metricName: monitoring_1.MetricType.Memory,
        operator: monitoring_1.AlertOperator.LessThan,
        threshold,
        severity,
        timeAggregation: monitoring_1.TimeAggregation.Average,
        windowSize: 'PT5M',
...
function addDiskAlerts(manager, vmName, threshold, severity, email) {
    // Disk Read alert
    const readRule = {
        name: 'high-disk-read',
        description: 'Alert when disk read is high',
        metricName: monitoring_1.MetricType.DiskRead,
        operator: monitoring_1.AlertOperator.GreaterThan,
        threshold: 1073741824, // 1GB in bytes
        severity,
        timeAggregation: monitoring_1.TimeAggregation.Total,
        windowSize: 'PT5M',
        evaluationFrequency: 'PT5M',
        enabled: true,
    };
    ...
    const writeRule = {
        ...
        metricName: monitoring_1.MetricType.DiskWrite,
        operator: monitoring_1.AlertOperator.GreaterThan,
        threshold: 1073741824, // 1GB in bytes
        severity,
        timeAggregation: monitoring_1.TimeAggregation.Total,
...
function addNetworkAlerts(manager, vmName, threshold, severity, email) {
    // Network In alert
    const inRule = {
        name: 'high-network-in',
        description: 'Alert when network ingress is high',
        metricName: monitoring_1.MetricType.NetworkIn,
        operator: monitoring_1.AlertOperator.GreaterThan,
        threshold: 1073741824, // 1GB in bytes
        severity,
        timeAggregation: monitoring_1.TimeAggregation.Total,
    ...
    const outRule = {
        ...
        metricName: monitoring_1.MetricType.NetworkOut,
        operator: monitoring_1.AlertOperator.GreaterThan,
        threshold: 1073741824, // 1GB in bytes
        severity,
        timeAggregation: monitoring_1.TimeAggregation.Total,
...
function mapSeverity(severity) {
    const severityMap = {
        'Critical': monitoring_1.AlertSeverity.Critical,
        'Error': monitoring_1.AlertSeverity.Error,
        'Warning': monitoring_1.AlertSeverity.Warning,
        'Informational': monitoring_1.AlertSeverity.Informational,
        'Verbose': monitoring_1.AlertSeverity.Verbose,
    };
    return severityMap[severity] || monitoring_1.AlertSeverity.Warning;
}
...
exports.default = configureMonitoringCommand;
