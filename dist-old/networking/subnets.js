...
exports.RESERVED_SUBNET_NAMES = exports.SUBNET_PATTERNS = void 0;
exports.validateSubnetName = validateSubnetName;
exports.validateReservedSubnet = validateReservedSubnet;
exports.getSubnetPattern = getSubnetPattern;
exports.getAllSubnetPatterns = getAllSubnetPatterns;
exports.createSubnetConfig = createSubnetConfig;
exports.subnetsOverlap = subnetsOverlap;
/**
 * Common subnet patterns for multi-tier architectures
 */
exports.SUBNET_PATTERNS = {
    /**
 ...
 */
function validateSubnetName(name) {
    ...
    if (!validNameRegex.test(name)) {
        return {
            valid: false,
            error: "Subnet name can only contain alphanumeric, underscore, hyphen, and period",
        };
    }
    ...
    if (name.startsWith(".") || name.endsWith(".")) {
        return {
            valid: false,
            error: "Subnet name cannot start or end with a period",
        };
    }
    return { valid: true };
}
 ...
 */
function validateReservedSubnet(name, addressPrefix) {
    const reserved = Object.values(exports.RESERVED_SUBNET_NAMES).find((r) => r.name === name);
    if (!reserved)
        return { valid: true };
 ...
 */
function getSubnetPattern(key) {
    return exports.SUBNET_PATTERNS[key];
}
/**
 * Get all subnet patterns
 */
function getAllSubnetPatterns() {
    return Object.entries(exports.SUBNET_PATTERNS).map(([key, pattern]) => ({
        key: key,
        pattern,
    }));
}
 ...
 */
function createSubnetConfig(name, addressPrefix, options) {
    return {
        name,
        addressPrefix,
 ...
 */
function subnetsOverlap(subnet1, subnet2) {
