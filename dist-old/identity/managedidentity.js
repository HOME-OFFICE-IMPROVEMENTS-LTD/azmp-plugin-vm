...
exports.managedidentity = exports.builtInRoles = void 0;
exports.systemAssignedIdentity = systemAssignedIdentity;
exports.userAssignedIdentity = userAssignedIdentity;
exports.multipleIdentities = multipleIdentities;
exports.createManagedIdentity = createManagedIdentity;
exports.getIdentityRecommendations = getIdentityRecommendations;
exports.validateManagedIdentityConfig = validateManagedIdentityConfig;
 ...
 */
function systemAssignedIdentity() {
    return {
        type: "SystemAssigned",
 ...
 */
function userAssignedIdentity(identityId) {
    return {
        type: "UserAssigned",
 ...
 */
function multipleIdentities(userIdentityIds) {
...
function createManagedIdentity(requirements) {
    // Multiple specific identities needed
    if (requirements.specificUserIdentities &&
        requirements.specificUserIdentities.length > 0) {
 ...
 */
function getIdentityRecommendations(useCase) {
    const recommendations = {
        ...
        storage: {
            ...
            additionalConfig: [
                ...
                "Use managed identity for storage account key",
            ],
        ...
        "multi-service": {
            ...
            additionalConfig: [
                "Use separate identities for different service groups",
 ...
 */
function validateManagedIdentityConfig(config) {
    const errors = [];
    const warnings = [];
    // Type validation
    if (!config.type) {
    ...
    if (config.type.includes("UserAssigned")) {
        if (!config.userAssignedIdentities ||
            Object.keys(config.userAssignedIdentities).length === 0) {
            errors.push("User-assigned identities must be specified when using UserAssigned type");
        }
    }
    // Performance warnings
    if (config.userAssignedIdentities &&
        Object.keys(config.userAssignedIdentities).length > 5) {
 ...
 */
function createRoleAssignment(identityPrincipalId, roleDefinitionId, scope) {
    return {
...
exports.managedidentity = {
    ...
    builtInRoles: exports.builtInRoles,
};
