...
const availabilityHelpers = __importStar(require("./availability/availabilitysets"));
const zoneHelpers = __importStar(require("./availability/availabilityzones"));
const vmssHelpers = __importStar(require("./availability/vmss"));
const backupHelpers = __importStar(require("./recovery/backup"));
const siteRecoveryHelpers = __importStar(require("./recovery/siterecovery"));
/**
 * VM Plugin Class
 *
...
class VmPlugin {
    ...
    async initialize(context) {
        ...
        (0, availability_1.registerAvailabilityHelpers)();
        (0, recovery_1.registerRecoveryHelpers)();
        (0, scaling_1.registerScalingHelpers)();
        (0, monitoring_1.registerMonitoringHelpers)();
        (0, monitoring_2.registerEnhancedMonitoringHelpers)();
        (0, alert_1.registerAlertHelpers)();
        (0, dashboard_1.registerDashboardHelpers)();
        (0, cost_1.registerCostHelpers)();
        (0, performance_1.registerPerformanceHelpers)();
    }
    /**
    ...
    getHandlebarsHelpers() {
        ...
        const networkingHelpers = (0, networking_1.getNetworkingHelpers)();
        // Get extension helpers with ext: namespace
        const extensionHelpers = (0, extensions_1.createExtensionHelpers)();
        // Get security helpers with security: namespace
        const securityHelpers = (0, security_1.createSecurityHelpers)();
        // Get identity helpers with identity: namespace
        const identityHelpers = (0, identity_1.createIdentityHelpers)();
        ...
        const vmHelpers = {
             ...
             */
            "vm-resource-name": (baseName, suffix) => {
    ...
    async loadJsonInput(input, label) {
        ...
        try {
            ...
            throw new Error(`Failed to parse ${label}: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
    }
    /**
    ...
    registerCommands(program) {
        ...
        vmCommand
            ...
            if (this.context) {
                ...
                try {
                    ...
                    const { azureAuth, ComputeHelper } = await Promise.resolve().then(() => __importStar(require("./azure")));
                    ...
                    if (sizes.length === 0) {
                        ...
                        sizes.slice(0, 20).forEach((size) => {
                            this.context.logger.info(`  ${size.name.padEnd(25)} - ${size.numberOfCores} cores, ${(size.memoryInMB / 1024).toFixed(1)}GB RAM, ${size.maxDataDiskCount} data disks`);
                        });
                        if (sizes.length > 20) {
        ...
        vmCommand
            ...
            if (this.context) {
                try {
                    ...
                    const { azureAuth } = await Promise.resolve().then(() => __importStar(require("./azure")));
        ...
        vmCommand
            ...
            .option("-c, --category <category>", "Filter by category (vm-monitoring, application, infrastructure, advanced-monitoring, scaling-analytics, cost-optimization)")
            .option("-t, --tags <tags>", "Filter by tags (comma-separated)")
            .action(async (options) => {
            if (this.context) {
                try {
                    const { WorkbookTemplateManager } = await Promise.resolve().then(() => __importStar(require("./workbooks")));
                    ...
                    if (options.category) {
                        ...
                        if (!validCategories.includes(categoryInput)) {
                            this.context.logger.warn(`⚠️  Unknown category "${options.category}". Supported categories: ${validCategories.join(", ")}`);
                            templates = [];
                        }
                        else {
                            templates =
                    ...
                    if (options.tags && templates.length > 0) {
                        const normalizedTags = options.tags
                            ...
                            .filter(Boolean);
                        templates = templates.filter((template) => normalizedTags.some((tag) => template.tags.includes(tag)));
                    }
                    if (templates.length === 0) {
                        this.context.logger.info("No workbook templates found matching the criteria");
                        return;
                    }
                    ...
                    for (const [category, categoryTemplates] of Object.entries(byCategory)) {
                        this.context.logger.info(`🔸 ${category.toUpperCase().replace("-", " ")}:`);
                        categoryTemplates.forEach((template) => {
                            if (template && this.context) {
                                this.context.logger.info(`   ${template.id.padEnd(25)} - ${template.name}`);
                                this.context.logger.info(`   ${" ".repeat(27)}${template.description}`);
                                this.context.logger.info(`   ${" ".repeat(27)}Tags: ${template.tags.join(", ")}\n`);
                            }
                        });
                    }
                    this.context.logger.info(`💡 Use "azmp vm wb-gen --template <id>" to generate a workbook`);
                }
                catch (error) {
                    this.context.logger.error(`Failed to list workbooks: ${error instanceof Error ? error.message : "Unknown error"}`);
                }
            }
        });
        vmCommand
            ...
            if (this.context) {
                try {
                    if (!options.template) {
                        this.context.logger.error("❌ Template ID is required. Use --template <id>");
                        this.context.logger.info('💡 Use "azmp vm wb-ls" to see available templates');
                        return;
                    }
                    const { WorkbookTemplateManager } = await Promise.resolve().then(() => __importStar(require("./workbooks")));
                    ...
                    if (options.parameters) {
                        try {
                            ...
                            this.context.logger.error("❌ Invalid JSON in --parameters option");
                            return;
                        }
                    }
                    // Generate workbook
                    ...
                    if (options.output) {
                        ...
                        this.context.logger.info(`✅ Workbook generated successfully: ${options.output}`);
                        this.context.logger.info(`📊 Template: ${workbook.metadata.templateName}`);
                    }
                    else {
                    ...
                    this.context.logger.error(`Failed to generate workbook: ${error instanceof Error ? error.message : "Unknown error"}`);
                }
            }
        });
        vmCommand
            ...
            if (this.context) {
                try {
                    if (!options.file) {
                        this.context.logger.error("❌ File path is required. Use --file <path>");
                        return;
                    }
                    ...
                    if (!Array.isArray(workbookData.items)) {
                        ...
                        // Validate items
                        workbookData.items.forEach((item, index) => {
                            if (typeof item.type !== "number") {
                                errors.push(`Item ${index}: missing or invalid type property`);
                            }
                            if (!item.content) {
                    ...
                    if (workbookData.metadata) {
                        if (workbookData.metadata.templateId) {
                            const template = WorkbookTemplateManager.getTemplate(workbookData.metadata.templateId);
                            if (!template) {
                                warnings.push(`Unknown template ID: ${workbookData.metadata.templateId}`);
                            }
                            else {
                                this.context.logger.info(`📊 Template: ${template.name} (${template.version})`);
                            }
                        }
                    }
                    // Report results
                    ...
                    if (isValid) {
                        ...
                        this.context.logger.info(`📊 Items: ${workbookData.items?.length || 0}`);
                    }
                    else {
                        this.context.logger.error("❌ Workbook validation failed");
                        if (this.context) {
                            errors.forEach((error) => this.context.logger.error(`   • ${error}`));
                        }
                    }
                    if (warnings.length > 0 && this.context) {
                        this.context.logger.warn("⚠️  Warnings:");
                        warnings.forEach((warning) => this.context.logger.warn(`   • ${warning}`));
                    }
                    if (options.verbose && isValid) {
                        ...
                        this.context.logger.info(`   Items: ${workbookData.items?.length || 0}`);
                        if (workbookData.metadata) {
                            this.context.logger.info(`   Generated: ${workbookData.metadata.generatedAt || "Unknown"}`);
                            this.context.logger.info(`   Generated by: ${workbookData.metadata.generatedBy || "Unknown"}`);
                        }
                    }
                }
                catch (error) {
                    this.context.logger.error(`Failed to validate workbook: ${error instanceof Error ? error.message : "Unknown error"}`);
                }
            }
        });
        // ========================================
        ...
        vmCommand
            .command("co-estimate")
            .description("Estimate VM cost for a specific size, region, and pricing model")
            .option("-s, --size <vmSize>", "Azure VM size (e.g., Standard_D2s_v3)")
            .option("-r, --region <region>", "Azure region", "eastus")
            .option("-p, --pricing <model>", "Pricing model (payg, reserved1year, reserved3year, spot)", "payg")
            .option("-h, --hours <hours>", "Number of hours (defaults to 730 for monthly)", "730")
            .option("-o, --os-type <type>", "Operating system type (linux or windows)", "linux")
            .option("--hybrid-benefit", "Apply Azure Hybrid Benefit (Windows licensing credit)")
            .action(async (options) => {
            if (!this.context)
                return;
            try {
                if (!options.size) {
                    this.context.logger.error("❌ VM size is required. Use --size <vmSize>");
                    return;
                }
                const { CostAnalyzer } = await Promise.resolve().then(() => __importStar(require("./cost")));
                ...
                this.context.logger.info(`   Pricing model: ${result.pricingModel.toUpperCase()} (${result.currency})`);
                this.context.logger.info(`   Hourly cost: $${result.hourlyCost.toFixed(4)}`);
                this.context.logger.info(`   Monthly cost (${result.hours}h): $${result.monthlyCost.toFixed(2)}`);
                if (options.osType === "windows") {
                    if (result.details.appliedHybridBenefit) {
                        this.context.logger.info("   Azure Hybrid Benefit applied (Windows licensing excluded)");
                    }
                    else {
                ...
                this.context.logger.error(`Failed to estimate VM cost: ${error instanceof Error ? error.message : "Unknown error"}`);
            }
        });
        vmCommand
            .command("co-compare")
            ...
            try {
                ...
                if (sizeList.length === 0) {
                    this.context.logger.error("❌ At least one VM size is required. Use --sizes <size1,size2,...>");
                    return;
                }
                const { CostAnalyzer } = await Promise.resolve().then(() => __importStar(require("./cost")));
                const hours = Number(options.hours);
                const pricingModel = (options.pricing || "payg").toLowerCase();
                const results = CostAnalyzer.compareVmSizes(sizeList, options.region, {
                    pricingModel: pricingModel,
                    hours: Number.isNaN(hours) ? undefined : hours,
                    osType: options.osType,
                    applyHybridBenefit: Boolean(options.hybridBenefit),
                ...
                results.forEach((entry) => {
                    this.context.logger.info(`   ${entry.vmSize.padEnd(20)} Hourly: $${entry.hourlyCost.toFixed(4)}  Monthly: $${entry.monthlyCost.toFixed(2)}`);
                });
            }
            catch (error) {
                this.context.logger.error(`Failed to compare VM sizes: ${error instanceof Error ? error.message : "Unknown error"}`);
            }
        });
        vmCommand
            .command("co-reserve")
            ...
            .option("-r, --region <region>", "Azure region", "eastus")
            .option("-t, --term <term>", "Reserved Instance term (1year or 3year)", "1year")
            .option("-o, --os-type <type>", "Operating system type (linux or windows)", "linux")
            .option("-h, --hours <hours>", "Number of hours (defaults to 730 for monthly)", "730")
            .action(async (options) => {
            if (!this.context)
            ...
            try {
                ...
                const hours = Number(options.hours);
                const { CostAnalyzer } = await Promise.resolve().then(() => __importStar(require("./cost")));
                const savings = CostAnalyzer.calculateReservedInstanceSavings(options.size, options.region, term, {
                    osType: options.osType,
                    hours: Number.isNaN(hours) ? undefined : hours,
                });
                this.context.logger.info(`\n🛒 Reserved Instance analysis for ${options.size} (${options.region})`);
                this.context.logger.info(`   Term: ${term === "1year" ? "1 year" : "3 year"}`);
                this.context.logger.info(`   PAYG monthly: $${savings.paygCost.toFixed(2)}`);
                this.context.logger.info(`   RI monthly:   $${savings.reservedCost.toFixed(2)}`);
                this.context.logger.info(`   Savings:      $${savings.savings.toFixed(2)} (${savings.savingsPercent.toFixed(2)}%)`);
            }
            catch (error) {
                this.context.logger.error(`Failed to calculate Reserved Instance savings: ${error instanceof Error ? error.message : "Unknown error"}`);
            }
        });
        vmCommand
            .command("co-rightsize")
            .description("Generate right-sizing recommendation from utilization metrics")
            .option("-c, --current-size <vmSize>", "Current VM size", "")
            .option("-r, --region <region>", "Azure region", "eastus")
            .option("--cpu-avg <percent>", "Average CPU utilization (0-100)", "35")
            .option("--memory-avg <percent>", "Average memory utilization (0-100)", "40")
            ...
            try {
                if (!options.currentSize) {
                    this.context.logger.error("❌ Current VM size is required. Use --current-size <vmSize>");
                    return;
                }
                const cpu = Number(options.cpuAvg);
                const memory = Number(options.memoryAvg);
                const disk = options.diskAvg !== undefined ? Number(options.diskAvg) : undefined;
                const hours = Number(options.hours);
                const { CostRecommendationEngine } = await Promise.resolve().then(() => __importStar(require("./cost")));
                const recommendation = CostRecommendationEngine.getRightSizingRecommendation({
                ...
                if (!recommendation) {
                    this.context.logger.info("ℹ️  No right-sizing recommendation. Current size appears appropriate for provided utilization.");
                    return;
                }
                this.context.logger.info(`\n📉 Right-sizing recommendation for ${options.currentSize}`);
                this.context.logger.info(`   Suggested size: ${recommendation.recommendedSize}`);
                this.context.logger.info(`   Monthly savings: $${recommendation.monthlySavings.toFixed(2)}`);
                this.context.logger.info(`   Annual savings:  $${recommendation.annualSavings.toFixed(2)}`);
                this.context.logger.info(`   Confidence: ${recommendation.confidence.toUpperCase()}`);
                this.context.logger.info(`   Rationale: ${recommendation.rationale}`);
            }
            catch (error) {
                this.context.logger.error(`Failed to generate right-sizing recommendation: ${error instanceof Error ? error.message : "Unknown error"}`);
            }
        });
        vmCommand
            .command("co-budget")
            .description("Generate a cost budget definition with optional alert template")
            .option("-n, --name <name>", "Budget name", "")
            .option("-a, --amount <amount>", "Budget amount in USD", "")
            .option("-g, --timegrain <grain>", "Time grain (Monthly, Quarterly, Annually)", "Monthly")
            .option("--start <date>", "Budget start date (YYYY-MM-DD)")
            .option("--end <date>", "Budget end date (optional)")
            .option("-c, --category <category>", "Budget category (Cost or Usage)", "Cost")
            .option("--notify <notification>", "Notification rule in format threshold:email1;email2 (repeatable)", (value, previous) => {
            const list = previous || [];
            list.push(value);
            return list;
        })
            .option("--action-group <group>", "Action group resource ID (repeatable)", (value, previous) => {
            const list = previous || [];
            list.push(value);
            return list;
        })
            ...
            try {
                if (!options.name) {
                    this.context.logger.error("❌ Budget name is required. Use --name <name>");
                    return;
                }
                if (!options.amount) {
                    this.context.logger.error("❌ Budget amount is required. Use --amount <value>");
                    return;
                }
                const amount = Number(options.amount);
                if (Number.isNaN(amount) || amount <= 0) {
                    this.context.logger.error("❌ Budget amount must be a positive number.");
                    return;
                }
                const { createBudgetDefinition, createCostAlertTemplate } = await Promise.resolve().then(() => __importStar(require("./cost")));
                const notifications = (options.notify || []).map((rule) => {
                ...
                if (notifications.length > 0) {
                    ...
                    this.context.logger.info("ℹ️  Add notifications with --notify 80:ops@example.com");
                }
            }
            catch (error) {
                this.context.logger.error(`Failed to generate budget information: ${error instanceof Error ? error.message : "Unknown error"}`);
            }
        });
        // ========================================
        ...
        vmCommand
            .command("pf-analyze")
            .description("Analyze VM performance metrics and generate optimization recommendations")
            .option("-s, --size <vmSize>", "Current VM size", "")
            .option("-m, --metrics <metrics>", "Performance metrics JSON string or path to JSON file", "")
            ...
            try {
                ...
                if (!options.metrics) {
                    this.context.logger.error("❌ Metrics input is required. Use --metrics <json|string>");
                    return;
                }
                const rawMetrics = await this.loadJsonInput(options.metrics, "metrics");
                const { PerformanceAnalyzer } = await Promise.resolve().then(() => __importStar(require("./performance")));
                const analysis = PerformanceAnalyzer.analyzePerformance(options.size, rawMetrics);
                if (options.output) {
                    const fs = await Promise.resolve().then(() => __importStar(require("fs/promises")));
                    await fs.writeFile(options.output, JSON.stringify(analysis, null, 2), "utf8");
                    this.context.logger.info(`✅ Performance analysis written to ${options.output}`);
                }
                this.context.logger.info(`\n⚙️  Performance analysis for ${analysis.vmSize}`);
                this.context.logger.info(`   Overall score: ${analysis.overallScore}/100`);
                this.context.logger.info(`   CPU utilization: ${analysis.utilizationAnalysis.cpuUtilization}`);
                this.context.logger.info(`   Memory utilization: ${analysis.utilizationAnalysis.memoryUtilization}`);
                this.context.logger.info(`   Disk utilization: ${analysis.utilizationAnalysis.diskUtilization}`);
                this.context.logger.info(`   Network utilization: ${analysis.utilizationAnalysis.networkUtilization}`);
                if (analysis.recommendations.length > 0) {
                    this.context.logger.info("   Top recommendation: " + analysis.recommendations[0].title);
                }
            }
            catch (error) {
                this.context.logger.error(`Failed to analyze performance: ${error instanceof Error ? error.message : "Unknown error"}`);
            }
        });
        vmCommand
            .command("pf-burst")
            .description("Evaluate burst capability and suitability for burst-capable VM sizes")
            .option("-s, --size <vmSize>", "Current VM size", "")
            .option("-m, --metrics <metrics>", "Performance metrics JSON string or path to JSON file", "")
            .action(async (options) => {
            if (!this.context)
            ...
            try {
                ...
                const rawMetrics = await this.loadJsonInput(options.metrics, "metrics");
                const { PerformanceAnalyzer } = await Promise.resolve().then(() => __importStar(require("./performance")));
                const burst = PerformanceAnalyzer.analyzeBurstCapability(options.size, rawMetrics);
                this.context.logger.info(`\n⚡ Burst analysis for ${burst.vmSize}`);
                this.context.logger.info(`   Supports burst: ${burst.supportsBurst ? "Yes" : "No"}`);
                if (burst.supportsBurst) {
                    this.context.logger.info(`   Burst sustainability: ${burst.burstConfiguration.sustainabilityMinutes.toFixed(1)} minutes`);
                }
                else if (burst.recommendations.targetVmSize) {
                    this.context.logger.info(`   Consider migrating to: ${burst.recommendations.targetVmSize}`);
                }
                this.context.logger.info(`   Suitability: ${burst.workloadSuitability.toUpperCase()}`);
                if (burst.recommendations.reasoning) {
                    this.context.logger.info(`   Recommendation: ${burst.recommendations.reasoning}`);
                }
            }
            catch (error) {
                this.context.logger.error(`Failed to evaluate burst capability: ${error instanceof Error ? error.message : "Unknown error"}`);
            }
        });
        vmCommand
            .command("pf-disk")
            .description("Analyze disk performance and recommend optimal storage tier")
            .option("-t, --tier <tier>", "Current disk tier (Standard_HDD, Standard_SSD, Premium_SSD, Ultra_SSD)", "")
            .option("-s, --size <vmSize>", "Current VM size", "")
            .option("-m, --metrics <metrics>", "Performance metrics JSON string or path to JSON file", "")
            .action(async (options) => {
            if (!this.context)
            ...
            try {
                if (!options.tier) {
                    this.context.logger.error("❌ Disk tier is required. Use --tier <currentTier>");
                    return;
                }
                if (!options.size) {
                    this.context.logger.error("❌ VM size is required. Use --size <vmSize>");
                ...
                const rawMetrics = await this.loadJsonInput(options.metrics, "metrics");
                const { PerformanceAnalyzer } = await Promise.resolve().then(() => __importStar(require("./performance")));
                const diskAnalysis = PerformanceAnalyzer.analyzeDiskPerformance(options.tier, rawMetrics, options.size);
                this.context.logger.info(`\n💾 Disk analysis for ${options.size}`);
                this.context.logger.info(`   Current tier: ${diskAnalysis.currentTier}`);
                this.context.logger.info(`   Recommended tier: ${diskAnalysis.recommendations.recommendedTier}`);
                this.context.logger.info(`   Reasoning: ${diskAnalysis.recommendations.reasoning}`);
                this.context.logger.info(`   Performance gain: ${diskAnalysis.recommendations.performanceGain}`);
                this.context.logger.info(`   Cost impact: ${diskAnalysis.recommendations.costImpact}`);
            }
            catch (error) {
                this.context.logger.error(`Failed to analyze disk performance: ${error instanceof Error ? error.message : "Unknown error"}`);
            }
        });
        vmCommand
            .command("pf-compare")
            ...
            .option("-p, --profile <profile>", "Workload profile (cpu-intensive, memory-intensive, io-intensive, balanced)", "balanced")
            .action(async (options) => {
            if (!this.context)
                return;
            try {
                ...
                const { PerformanceAnalyzer } = await Promise.resolve().then(() => __importStar(require("./performance")));
                const comparison = PerformanceAnalyzer.compareVmPerformance(sizeList, options.profile);
                this.context.logger.info(`\n📈 VM performance comparison for profile "${options.profile}"`);
                comparison.slice(0, 5).forEach((result, index) => {
                    this.context.logger.info(`   ${index + 1}. ${result.vmSize} (score: ${result.suitabilityScore.toFixed(1)})`);
                    this.context.logger.info(`      Pros: ${result.pros.join("; ")}`);
                    if (result.cons.length > 0) {
                        this.context.logger.info(`      Cons: ${result.cons.join("; ")}`);
                    }
                });
            }
            catch (error) {
                this.context.logger.error(`Failed to compare VM performance: ${error instanceof Error ? error.message : "Unknown error"}`);
            }
        });
        vmCommand
            .command("pf-load")
            .description("Analyze workload metrics to detect scaling patterns")
            .option("-m, --metrics <metrics>", "Array of metrics with timestamp,cpuPercent,memoryPercent (JSON string or file)", "")
            ...
            try {
                ...
                if (!Array.isArray(rawMetrics)) {
                    this.context.logger.error("❌ Metrics input must be an array of records.");
                    return;
                }
                const normalized = rawMetrics.map((entry) => ({
                    ...entry,
                    timestamp: entry.timestamp ? new Date(entry.timestamp) : new Date(),
                }));
                const { AutoscaleEngine } = await Promise.resolve().then(() => __importStar(require("./performance")));
                const pattern = AutoscaleEngine.analyzeLoadPattern(normalized);
                if (options.output) {
                    const fs = await Promise.resolve().then(() => __importStar(require("fs/promises")));
                    await fs.writeFile(options.output, JSON.stringify(pattern, null, 2), "utf8");
                    this.context.logger.info(`✅ Load pattern written to ${options.output}`);
                }
                this.context.logger.info(`\n📊 Load pattern detected: ${pattern.patternType.toUpperCase()}`);
                this.context.logger.info(`   Confidence: ${pattern.confidence}%`);
                this.context.logger.info(`   Average load: ${pattern.characteristics.averageLoad.toFixed(1)}%`);
                this.context.logger.info(`   Peak load: ${pattern.characteristics.peakLoad.toFixed(1)}%`);
                this.context.logger.info(`   Recommended instances: ${pattern.scalingRecommendations.recommendedMinInstances}-${pattern.scalingRecommendations.recommendedMaxInstances}`);
            }
            catch (error) {
                this.context.logger.error(`Failed to analyze load pattern: ${error instanceof Error ? error.message : "Unknown error"}`);
            }
        });
        vmCommand
            ...
            .option("-p, --pattern <pattern>", "Load pattern JSON (string or file)", "")
            .option("--predictive", "Enable predictive autoscale")
            .option("--cost", "Optimize for cost (slower scale-out)")
            .option("--performance", "Optimize for performance (more aggressive scaling)")
            .option("--notify <email>", "Notification email (repeatable)", (value, previous = []) => {
            previous.push(value);
            return previous;
        })
            .option("-o, --output <file>", "Write autoscale configuration to JSON file")
            .action(async (options) => {
            if (!this.context)
                return;
            try {
                if (!options.resourceUri) {
                    this.context.logger.error("❌ Resource URI is required. Use --resource-uri <uri>");
                    return;
                }
                if (!options.pattern) {
                    this.context.logger.error("❌ Load pattern input is required. Use --pattern <json|string>");
                    return;
                }
                const loadPattern = await this.loadJsonInput(options.pattern, "pattern");
                const { AutoscaleEngine } = await Promise.resolve().then(() => __importStar(require("./performance")));
                const config = AutoscaleEngine.generateAutoscaleConfiguration(options.resourceUri, loadPattern, {
                ...
                if (options.output) {
                    const fs = await Promise.resolve().then(() => __importStar(require("fs/promises")));
                    await fs.writeFile(options.output, JSON.stringify(config, null, 2), "utf8");
                    this.context.logger.info(`✅ Autoscale configuration written to ${options.output}`);
                }
                ...
                if (config.profiles[0]) {
                    ...
                    this.context.logger.info(`   Default capacity: ${capacity.minimum}-${capacity.maximum} (default ${capacity.default})`);
                    this.context.logger.info(`   Rules: ${config.profiles[0].rules.length}`);
                }
                if (config.predictiveAutoscalePolicy) {
                    this.context.logger.info(`   Predictive scaling: ${config.predictiveAutoscalePolicy.scaleMode}`);
                }
            }
            catch (error) {
                this.context.logger.error(`Failed to generate autoscale configuration: ${error instanceof Error ? error.message : "Unknown error"}`);
            }
        });
        vmCommand
            .command("pf-simulate")
            .description("Simulate autoscale behaviour against historical metrics")
            .option("-m, --metrics <metrics>", "Array of metrics with timestamp,cpuPercent,instances (JSON string or file)", "")
            .option("-c, --config <config>", "Autoscale configuration JSON (string or file)", "")
            .option("--vm-size <vmSize>", "VM size for cost modelling", "Standard_D2s_v3")
            ...
            try {
                ...
                if (!options.config) {
                    this.context.logger.error("❌ Autoscale configuration is required. Use --config <json|string>");
                    return;
                }
                const metricsRaw = await this.loadJsonInput(options.metrics, "metrics");
                ...
                const { AutoscaleEngine } = await Promise.resolve().then(() => __importStar(require("./performance")));
                const simulation = AutoscaleEngine.simulateScaling(metrics, config, options.vmSize);
                if (options.output) {
                    const fs = await Promise.resolve().then(() => __importStar(require("fs/promises")));
                    await fs.writeFile(options.output, JSON.stringify(simulation, null, 2), "utf8");
                    this.context.logger.info(`✅ Simulation results written to ${options.output}`);
                }
                this.context.logger.info(`\n🧪 Autoscale simulation summary`);
                this.context.logger.info(`   Scale events: ${simulation.summary.totalScaleEvents}`);
                this.context.logger.info(`   Average instances: ${simulation.summary.averageInstances.toFixed(2)}`);
                this.context.logger.info(`   Total cost (simulated): $${simulation.summary.totalCost.toFixed(2)}`);
                this.context.logger.info(`   Performance score: ${simulation.summary.performanceScore.toFixed(1)}`);
                this.context.logger.info(`   Efficiency score: ${simulation.summary.efficiencyScore.toFixed(1)}`);
            }
            catch (error) {
                this.context.logger.error(`Failed to simulate autoscale behaviour: ${error instanceof Error ? error.message : "Unknown error"}`);
            }
        });
        // ========================================
        ...
        vmCommand
            ...
            .requiredOption("--resource-id <id>", "Target resource ID (e.g., VM resource ID)")
            .option("--threshold <percent>", "CPU percentage threshold", "80")
            .option("--severity <0-4>", "Alert severity (0=Sev0, 4=Informational)", "2")
            .option("--frequency <duration>", "Evaluation frequency (ISO 8601 duration)", "PT1M")
            .option("--window <duration>", "Alert evaluation window (ISO 8601 duration)", "PT5M")
            .option("--action-group <id>", "Action group resource ID (repeatable)", collectValues)
            .option("--email <address>", "Notification email (repeatable)", collectValues)
            .option("--output <file>", "Write alert definition JSON to file")
            .option("--arm", "Emit Azure metric alert resource payload instead of logical definition")
            .action(async (options) => {
            if (!this.context)
                return;
            try {
                ...
                const { MonitoringAlertEngine } = await Promise.resolve().then(() => __importStar(require("./monitoring")));
                ...
                if (options.output) {
                    ...
                    this.context.logger.info(`✅ Alert definition written to ${options.output}`);
                }
                else {
                    this.context.logger.info(JSON.stringify(outputPayload, null, 2));
                }
                this.context.logger.info(`⚙️  CPU alert ready for resource ${options.resourceId}`);
                this.context.logger.info(`   Threshold: ${threshold}% | Severity: ${severity}`);
                this.context.logger.info(`   Window: ${options.window} | Frequency: ${options.frequency}`);
            }
            catch (error) {
                this.context.logger.error(`Failed to generate CPU alert: ${error instanceof Error ? error.message : "Unknown error"}`);
            }
        });
        vmCommand
            ...
            .option("--threshold-mb <value>", "Available memory threshold (MB)", "512")
            .option("--severity <0-4>", "Alert severity (0=Sev0, 4=Informational)", "3")
            .option("--frequency <duration>", "Evaluation frequency (ISO 8601 duration)", "PT5M")
            .option("--window <duration>", "Alert evaluation window (ISO 8601 duration)", "PT10M")
            .option("--action-group <id>", "Action group resource ID (repeatable)", collectValues)
            .option("--email <address>", "Notification email (repeatable)", collectValues)
            .option("--output <file>", "Write alert definition JSON to file")
            .option("--arm", "Emit Azure metric alert resource payload instead of logical definition")
            ...
            if (!this.context)
                return;
            try {
                const thresholdMb = Number(options.thresholdMb ||
                    options.thresholdmb ||
                ...
                this.context.logger.info(`🧠 Memory alert ready for resource ${options.resourceId}`);
                this.context.logger.info(`   Threshold: ${thresholdMb} MB | Severity: ${severity}`);
            }
            catch (error) {
                this.context.logger.error(`Failed to generate memory alert: ${error instanceof Error ? error.message : "Unknown error"}`);
            }
        });
        vmCommand
            ...
            .option("--frequency <duration>", "Evaluation frequency (ISO 8601 duration)", "PT30M")
            .option("--window <duration>", "Evaluation window (ISO 8601 duration)", "P1D")
            .option("--action-group <id>", "Action group resource ID (repeatable)", collectValues)
            .option("--email <address>", "Notification email (repeatable)", collectValues)
            .option("--workspace-id <id>", "Log Analytics workspace resource ID (required for --arm output)")
            .option("--output <file>", "Write alert definition JSON to file")
            ...
            try {
                ...
                if (options.arm) {
                    if (!options.workspaceId) {
                        this.context.logger.error("❌ --workspace-id is required when using --arm");
                        return;
                    }
                    outputPayload = MonitoringAlertEngine.toScheduledQueryResource(definition, options.workspaceId);
                }
                if (options.output) {
                    const fs = await Promise.resolve().then(() => __importStar(require("fs/promises")));
                    await fs.writeFile(options.output, JSON.stringify(outputPayload, null, 2), "utf8");
                ...
                this.context.logger.info(`💰 Cost anomaly alert ready for scope ${options.scopeId}`);
                this.context.logger.info(`   Threshold: ${thresholdPercent}% | Severity: ${severity}`);
            }
            catch (error) {
                this.context.logger.error(`Failed to generate cost anomaly alert: ${error instanceof Error ? error.message : "Unknown error"}`);
            }
        });
        vmCommand
            .command("mon-alert-scaling")
            .description("Generate a scaling failure alert definition for VM scale sets")
            .requiredOption("--resource-id <id>", "VMSS resource ID")
            .option("--failures <count>", "Number of failures in the window to trigger", "3")
            .option("--severity <0-4>", "Alert severity", "2")
            .option("--frequency <duration>", "Evaluation frequency (ISO 8601 duration)", "PT5M")
            .option("--window <duration>", "Evaluation window (ISO 8601 duration)", "PT30M")
            .option("--action-group <id>", "Action group resource ID (repeatable)", collectValues)
            .option("--email <address>", "Notification email (repeatable)", collectValues)
            .option("--workspace-id <id>", "Log Analytics workspace resource ID (required for --arm output)")
            .option("--output <file>", "Write alert definition JSON to file")
            ...
            try {
                ...
                this.context.logger.info(`🚨 Scaling alert ready for ${options.resourceId}`);
                this.context.logger.info(`   Failure threshold: ${failureThreshold} | Severity: ${severity}`);
            }
            catch (error) {
                this.context.logger.error(`Failed to generate scaling alert: ${error instanceof Error ? error.message : "Unknown error"}`);
            }
        });
        // ========================================
        ...
        extCommand
            ...
            if (!this.context)
                return;
            const { extensionsCatalog } = require("./extensions");
            const extensions = extensionsCatalog.filter((ext) => ext.category === "windows");
        ...
        extCommand
            ...
            if (!this.context)
                return;
            const { extensionsCatalog } = require("./extensions");
            const extensions = extensionsCatalog.filter((ext) => ext.category === "linux");
        ...
        extCommand
            ...
            if (!this.context)
                return;
            const { extensionsCatalog } = require("./extensions");
            const extensions = extensionsCatalog.filter((ext) => ext.category === "crossplatform");
        ...
        securityCommand
            ...
            securityTemplates.forEach((template, index) => {
...
exports.VmPlugin = VmPlugin;
/**
 * Default export - plugin instance
 */
exports.default = VmPlugin;
