...
const toScheduledQueryResource = (
  definition: AlertRuleDefinition,
  workspaceId: string,
): Handlebars.SafeString => {
...
export function registerEnhancedMonitoringHelpers(): void {
  Handlebars.registerHelper("monitor:cpuAlert", function (options: any) {
    try {
      const alertOptions = parseOptions<CpuAlertOptions>(options.hash, [
        "resourceId",
      ]);
  ...
  Handlebars.registerHelper("monitor:memoryAlert", function (options: any) {
    try {
      const alertOptions = parseOptions<MemoryAlertOptions>(options.hash, [
        "resourceId",
      ]);
  ...
  Handlebars.registerHelper("monitor:costAlert", function (options: any) {
    try {
      const alertOptions = parseOptions<CostAnomalyAlertOptions>(options.hash, [
        "scopeId",
      ]);
      const definition =
        MonitoringAlertEngine.createCostAnomalyAlert(alertOptions);
      return safeStringify(definition);
    } catch (error) {
  ...
  Handlebars.registerHelper("monitor:scalingAlert", function (options: any) {
    try {
      const alertOptions = parseOptions<ScalingHealthAlertOptions>(
        options.hash,
        ["resourceId"],
      );
      const definition =
        MonitoringAlertEngine.createScalingHealthAlert(alertOptions);
      return safeStringify(definition);
    } catch (error) {
  ...
  Handlebars.registerHelper(
    "monitor:metricResource",
    function (definitionJson: string) {
      try {
        const definition =
          typeof definitionJson === "string"
            ? (JSON.parse(definitionJson) as AlertRuleDefinition)
            : definitionJson;
  ...
  Handlebars.registerHelper(
    "monitor:logResource",
    function (definitionJson: string, workspaceId: string) {
      try {
        if (!workspaceId) {
