 ...
 */
export function userAssignedIdentity(
  identityId: string,
 ...
 */
export function multipleIdentities(
  ...
  return {
    ...
    userAssignedIdentities,
    description:
      "Hybrid approach with both system and user-assigned identities",
    benefits: [
...
export function createManagedIdentity(
  ...
  if (
    requirements.specificUserIdentities &&
  ...
  if (requirements.sharedAcrossResources) {
    ...
    return {
      ...
      userAssignedIdentities: {},
      recommendation:
        "Create user-assigned identity resource first, then assign to VM",
    } as any;
  }
 ...
 */
export function validateManagedIdentityConfig(config: ManagedIdentityConfig): {
  valid: boolean;
  errors: string[];
  warnings: string[];
} {
  ...
  if (config.type.includes("UserAssigned")) {
    if (
      !config.userAssignedIdentities ||
      Object.keys(config.userAssignedIdentities).length === 0
    ) {
      errors.push(
        "User-assigned identities must be specified when using UserAssigned type",
      );
    }
  }
  ...
  if (
    ...
    warnings.push(
      "Consider using fewer identities for better performance and management",
    );
  }
