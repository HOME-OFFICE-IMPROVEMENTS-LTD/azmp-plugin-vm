 ...
 */
export function assignBuiltInRole(
  principalId: string,
  roleName: string,
  scopeType: RBACScope = "resource",
  scopeId?: string,
 ...
 */
export function createCustomRole(roleDefinition: CustomRoleDefinition) {
  return {
 ...
 */
export function roleAssignmentTemplate(assignment: RoleAssignmentConfig) {
  const template: any = {
 ...
 */
export function recommendRole(requiredActions: string[]): {
  roleName: string;
  roleId: string;
  reason: string;
  alternatives?: Array<{ name: string; id: string }>;
} {
  // VM read-only
  if (
    requiredActions.every((action) =>
      action.startsWith("Microsoft.Compute/*/read"),
    )
  ) {
    return {
      roleName: "Reader",
  ...
  // VM login
  if (
    requiredActions.includes("Microsoft.Compute/virtualMachines/login/action")
  ) {
    return {
  ...
  // Key Vault access
  if (
    requiredActions.some((action) => action.startsWith("Microsoft.KeyVault/"))
  ) {
    return {
  ...
  // Storage access
  if (
    requiredActions.some((action) => action.startsWith("Microsoft.Storage/"))
  ) {
    return {
  ...
  return {
    ...
    roleId: "acdd72a7-3385-48ef-bd42-f606fba81ae7",
    reason:
      "Default safe role - consider creating custom role for specific needs",
  };
}
 ...
 */
export const customRoleTemplates = {
  /**
   * VM Start/Stop Only
   */
  vmStartStop: (): CustomRoleDefinition => ({
    ...
    description:
      "Can start and stop virtual machines but cannot modify configuration",
    actions: [
 ...
 */
export function validateRBACConfig(assignment: RoleAssignmentConfig): {
  valid: boolean;
  errors: string[];
  warnings: string[];
} {
  ...
  if (assignment.condition && !assignment.conditionVersion) {
    warnings.push(
      "Condition version should be specified when using conditional access",
    );
  }
  ...
  if (assignment.roleDefinitionId === "8e3af657-a8ff-443c-a75c-2fe8c4bcb635") {
    // Owner role
    warnings.push(
      "Owner role grants full access - consider using a more restrictive role",
    );
  }
 ...
 */
export function getRBACBestPractices() {
  return {
    principles: [
