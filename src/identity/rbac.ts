...
export function assignBuiltInRole(
  ...
  switch (scopeType) {
    case 'resource':
      scope = scopeId || '[resourceId(\'Microsoft.Compute/virtualMachines\', parameters(\'vmName\'))]';
      break;
    case 'resourceGroup':
 ...
 */
export function createScope(scopeType: RBACScope, resourceName?: string): string {
  switch (scopeType) {
    case 'resource':
      if (!resourceName) {
...
export function recommendRole(requiredActions: string[]): {
  ...
  if (requiredActions.includes('Microsoft.Compute/virtualMachines/login/action')) {
    return {
      ...
      alternatives: [
        { name: 'Virtual Machine Administrator Login', id: '1c0163c0-47e6-4577-8991-ea5c82e286e4' }
      ]
    };
  }
...
export function validateRBACConfig(
  assignment: RoleAssignmentConfig
  ...
  if (!assignment.roleDefinitionId || assignment.roleDefinitionId.trim() === '') {
  ...
  if (assignment.roleDefinitionId === '8e3af657-a8ff-443c-a75c-2fe8c4bcb635') { // Owner role
