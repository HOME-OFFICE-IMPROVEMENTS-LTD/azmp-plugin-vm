 ...
 */
export function aadSSHLoginExtension(config: AADSSHLoginConfig = { enabled: true }) {
  return {
 ...
 */
export function aadLoginForWindows(config: AADSSHLoginConfig = { enabled: true }) {
  return {
    type: 'Microsoft.Compute/virtualMachines/extensions',
    apiVersion: '2023-03-01',
 ...
 */
export function conditionalAccessPolicy(policy: ConditionalAccessPolicy) {
  return {
    name: policy.name,
 ...
 */
export function mfaConfiguration(config: MFAConfig) {
  return {
    enabled: config.enabled,
    ...
    features: {
      phoneAuth: config.methods.includes('phone') ? 'SMS or phone call authentication' : null,
      emailAuth: config.methods.includes('email') ? 'Email verification code' : null,
      authenticatorApp: config.methods.includes('authenticator') ? 'Microsoft Authenticator or other TOTP apps' : null,
      fido2: config.methods.includes('fido2') ? 'Hardware security keys (FIDO2)' : null,
      windowsHello: config.methods.includes('windowsHello') ? 'Windows Hello biometric authentication' : null
    },
    recommendations: [
...
export function passwordlessAuthentication(
  methods: Array<'fido2' | 'windowsHello' | 'authenticator'> = ['fido2', 'authenticator']
) {
  return {
    enabled: true,
    methods,
    features: {
      ...
      authenticator: methods.includes('authenticator') ? {
        ...
        requirements: ['Microsoft Authenticator app', 'Phone sign-in enabled']
      } : null
    },
    benefits: [
 ...
 */
export function vmAccessRole(roleType: 'Administrator' | 'User') {
  const roles = {
    Administrator: {
...
export function createAADIntegration(
  platform: 'Windows' | 'Linux',
  features: AADIntegrationFeatures = {}
) {
  const config: any = {
    platform,
    features: {}
  ...
  if (features.enableSSHLogin !== false) {
    config.features.aadLogin = platform === 'Linux'
      ? aadSSHLoginExtension({
          enabled: true,
          allowedPrincipals: [...(features.administratorPrincipals || []), ...(features.userPrincipals || [])],
  ...
  if (features.enablePasswordless) {
    config.features.passwordless = passwordlessAuthentication(
      platform === 'Windows'
 ...
 */
export function validateAADConfig(
  config: any
  ...
  if (config.features?.aadLogin) {
    if (config.platform === 'Linux' && !config.features.aadLogin.properties?.type?.includes('Linux')) {
      errors.push('Linux platform requires AADSSHLoginForLinux extension');
    }
    if (config.platform === 'Windows' && !config.features.aadLogin.properties?.type?.includes('Windows')) {
  ...
  if (config.features?.conditionalAccess && !config.features?.conditionalAccess.enabled) {
