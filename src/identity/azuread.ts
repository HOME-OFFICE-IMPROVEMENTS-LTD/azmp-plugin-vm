...
export interface MFAConfig {
  enabled: boolean;
  methods: Array<
    "phone" | "email" | "authenticator" | "fido2" | "windowsHello"
  >;
 ...
 */
export function aadSSHLoginExtension(
  config: AADSSHLoginConfig = { enabled: true },
) {
  return {
 ...
 */
export function aadLoginForWindows(
  config: AADSSHLoginConfig = { enabled: true },
) {
  return {
    type: "Microsoft.Compute/virtualMachines/extensions",
  ...
  return {
    ...
    controls: {
      ...
      requireDomainJoinedDevice:
        policy.controls.requireDomainJoinedDevice === true,
...
export function createAADIntegration(
  ...
  config.roleAssignments = {
    administrators:
      features.administratorPrincipals?.map((principal) => ({
        principalId: principal,
        role: vmAccessRole("Administrator"),
      })) || [],
    users:
      features.userPrincipals?.map((principal) => ({
        principalId: principal,
 ...
 */
export function validateAADConfig(config: any): {
  valid: boolean;
  errors: string[];
  warnings: string[];
} {
  ...
  if (config.features?.aadLogin && !config.managedIdentity) {
    warnings.push(
      "AAD Login requires managed identity - ensure VM has system or user-assigned identity",
    );
  }

  // Conditional Access validation
  if (
    config.features?.conditionalAccess &&
