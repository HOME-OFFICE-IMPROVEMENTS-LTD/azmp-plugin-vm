 ...
 */
export function createVmssDefinition(options: VmssDefinitionOptions): Record<string, unknown> {
  if (!options?.name) {
  ...
  const orchestrationMode: VmssOrchestrationMode = options.orchestrationMode ?? 'Uniform';
  ...
  if (orchestrationMode === 'Flexible') {
    ...
    resource.properties.singlePlacementGroup = options.singlePlacementGroup ?? false;
  } else {
    const faultDomains = options.platformFaultDomainCount ?? 2;
    ...
    resource.properties.singlePlacementGroup = options.singlePlacementGroup ?? true;
    resource.properties.platformFaultDomainCount = faultDomains;
