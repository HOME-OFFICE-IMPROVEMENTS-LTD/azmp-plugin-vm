...
export function registerCostHelpers(): void {
  ...
  Handlebars.registerHelper('cost:reservedInstanceSavings', function(...args: any[]) {
    try {
      const { vmSize, options } = resolveOptions(args);
      const hash = options?.hash || {};
      const size = vmSize || hash.vmSize;
      ...
      const result = CostAnalyzer.calculateReservedInstanceSavings(size, region, term, {
        osType: hash.osType,
        hours: hash.hours
      });
  ...
  Handlebars.registerHelper('cost:rightSizeRecommendation', function(...args: any[]) {
    try {
      const { vmSize, options } = resolveOptions(args);
      const hash = options?.hash || {};
      ...
      const recommendation = CostRecommendationEngine.getRightSizingRecommendation({
        ...
        avgDiskPercent: hash.avgDiskPercent !== undefined ? Number(hash.avgDiskPercent) : undefined,
        osType: hash.osType,
  ...
  Handlebars.registerHelper('cost:storageCost', function(...args: any[]) {
    try {
      ...
      const sizeGb = sizeArg !== undefined ? Number(sizeArg) : Number(hash?.sizeGb ?? hash?.size);
  ...
  Handlebars.registerHelper('cost:spotInstanceSavings', function(...args: any[]) {
    try {
      const { vmSize, options } = resolveOptions(args);
      const hash = options?.hash || {};
      const size = vmSize || hash.vmSize;
  ...
  Handlebars.registerHelper('cost:hybridBenefitSavings', function(...args: any[]) {
    try {
      const { vmSize, options } = resolveOptions(args);
      const hash = options?.hash || {};
      const size = vmSize || hash.vmSize;
      ...
      const result = CostAnalyzer.calculateHybridBenefitSavings(size, region, hours);
      return safeStringify(result);
    } catch (error) {
  ...
  Handlebars.registerHelper('cost:forecast', function(options: any) {
    try {
      ...
      const result = CostAnalyzer.forecastCosts(currentMonthlyCost, growthRate, months);
      return safeStringify(result);
    } catch (error) {
