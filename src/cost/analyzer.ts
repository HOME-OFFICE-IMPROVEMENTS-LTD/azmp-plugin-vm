...
export class CostAnalyzer {
  ...
  static calculateVmCost(options: VmCostOptions): VmCostBreakdown {
    ...
    const windowsPremiumHourly =
      osType === "windows" ? (pricing.windowsPremium ?? 0) : 0;

    const appliedHybridBenefit =
      osType === "windows" && options.applyHybridBenefit === true;
   ...
   */
  static compareVmSizes(
    sizes: string[],
    region: string,
    options: Partial<Omit<VmCostOptions, "vmSize" | "region">> = {},
  ): VmCostComparison[] {
   ...
   */
  static calculateReservedInstanceSavings(
    vmSize: string,
    region: string,
    term: "1year" | "3year",
    options: { osType?: "linux" | "windows"; hours?: number } = {},
    ...
    const reservedRate =
      term === "1year" ? pricing.reserved1Year : pricing.reserved3Year;
    const windowsPremium =
      options.osType === "windows" ? (pricing.windowsPremium ?? 0) : 0;
    ...
    const savingsPercent = Number(
      ((savings / payg.monthlyCost) * 100).toFixed(2),
    );
   ...
   */
  static calculateSpotInstanceSavings(
    vmSize: string,
    region: string,
    options: { hours?: number; osType?: "linux" | "windows" } = {},
  ): SpotInstanceSavings {
    const hours = options.hours ?? DEFAULT_HOURS_PER_MONTH;
    const pricing = CostAnalyzer.getVmPricing(vmSize, region);
   ...
   */
  static calculateHybridBenefitSavings(
    vmSize: string,
    region: string,
    hours: number = DEFAULT_HOURS_PER_MONTH,
  ): HybridBenefitSavings {
    const pricing = CostAnalyzer.getVmPricing(vmSize, region);
    const windowsPremium = pricing.windowsPremium ?? 0;

    if (windowsPremium === 0) {
      throw new Error(
        `Hybrid Benefit savings not applicable for ${vmSize} (no Windows premium)`,
      );
    }
    ...
    const windowsWithoutAhb =
      linuxCost.monthlyCost + Number((windowsPremium * hours).toFixed(2));
    const windowsWithAhb = linuxCost.monthlyCost;
    const savings = Number((windowsWithoutAhb - windowsWithAhb).toFixed(2));
    const savingsPercent = Number(
      ((savings / windowsWithoutAhb) * 100).toFixed(2),
    );
  ...
  private static getVmPricing(vmSize: string, region: string): VmPricingEntry {
    ...
    if (!pricing) {
      throw new Error(
        `Pricing data unavailable for VM size ${vmSize} in region ${region}`,
      );
    }
  ...
  private static getHourlyRate(
    pricing: VmPricingEntry,
    model: VmPricingModel,
  ): number {
    switch (model) {
      case "payg":
        return pricing.payg;
