 ...
 */
export function createBudgetDefinition(options: BudgetDefinitionOptions): BudgetDefinition {
  if (!options.name) {
  ...
  const notifications = (options.notifications ?? []).reduce<Record<string, any>>((acc, notification, index) => {
  ...
  if (options.tags && Object.keys(options.tags).length > 0) {
    filters.tags = Object.entries(options.tags).reduce<Record<string, string[]>>((acc, [key, value]) => {
      acc[key] = [value];
      return acc;
    }, {});
  }
 ...
 */
export function createCostAlertTemplate(options: CostAlertTemplateOptions): CostAlertTemplate {
  ...
  return {
    ...
    properties: {
      ...
      notification: {
        ...
        message: options.message ?? `Budget ${options.budgetName} exceeded ${options.threshold}%`
      },
