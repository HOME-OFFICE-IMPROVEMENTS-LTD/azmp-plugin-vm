...
describe('Cost Optimization Module', () => {
  ...
  describe('CostAnalyzer', () => {
    ...
    it('compares VM sizes across a region', () => {
      const results = CostAnalyzer.compareVmSizes(['Standard_D2s_v3', 'Standard_D4s_v3'], 'eastus', {
        pricingModel: 'payg',
        hours: 730
      });
    ...
    it('calculates reserved instance savings', () => {
      const savings = CostAnalyzer.calculateReservedInstanceSavings('Standard_D2s_v3', 'eastus', '1year');
    ...
    it('calculates spot instance savings', () => {
      const savings = CostAnalyzer.calculateSpotInstanceSavings('Standard_D2s_v3', 'eastus');
    ...
    it('throws for unsupported disk type', () => {
      expect(() => CostAnalyzer.calculateStorageCost('Unknown_SKU', 128)).toThrow('Unsupported disk type');
    });

    it('calculates hybrid benefit savings', () => {
      const savings = CostAnalyzer.calculateHybridBenefitSavings('Standard_D2s_v3', 'eastus');
      expect(savings.savings).toBeGreaterThan(0);
      expect(savings.withoutHybridBenefit).toBeGreaterThan(savings.withHybridBenefit);
    });

    it('generates cost forecast', () => {
      const forecast = CostAnalyzer.forecastCosts(500, 0.05, 3);
      expect(forecast).toHaveLength(3);
      expect(forecast[2].projectedCost).toBeGreaterThan(forecast[0].projectedCost);
    });
  });
  ...
  describe('Budget helpers', () => {
    ...
    it('creates cost alert template', () => {
      ...
      expect(alert.properties.notification.contactEmails).toContain('ops@example.com');
    });
  });

  describe('Handlebars helpers', () => {
    it('renders cost:calculateVmCost helper output', () => {
      const template = Handlebars.compile(`{{cost:calculateVmCost vmSize="Standard_D2s_v3" region="eastus"}}`);
