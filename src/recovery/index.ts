...
export function registerRecoveryHelpers() {
  ...
  Handlebars.registerHelper('recovery:backupPreset', function(preset: string, vaultName: string) {
  ...
  Handlebars.registerHelper('recovery:estimateBackupStorage', function(
    vmSizeGB: number,
    dailyRetention: number,
    weeklyRetention: number,
    monthlyRetention: number
  ) {
    return estimateBackupStorage(vmSizeGB, dailyRetention, weeklyRetention, monthlyRetention);
  });

  // ===== Site Recovery Helpers =====
  
  Handlebars.registerHelper('recovery:replicationPolicy', function(options: any) {
  ...
  Handlebars.registerHelper('recovery:enableReplication', function(options: any) {
  ...
  Handlebars.registerHelper('recovery:estimateRTO', function(vmCount: number, avgVMSize: number) {
    return estimateRTO(vmCount, avgVMSize);
  });

  Handlebars.registerHelper('recovery:estimateRPO', function(crashConsistentFrequency: number) {
    return estimateRPO(crashConsistentFrequency);
  });

  Handlebars.registerHelper('recovery:estimateBandwidth', function(vmSizeGB: number, changeRate: number) {
  ...
  Handlebars.registerHelper('recovery:restorePointCollection', function(options: any) {
  ...
  Handlebars.registerHelper('recovery:diskFromSnapshot', function(
    diskName: string,
    snapshotName: string,
    location?: string
  ) {
    return JSON.stringify(diskFromSnapshot(diskName, snapshotName, location), null, 2);
  });

  Handlebars.registerHelper('recovery:estimateSnapshotCost', function(
    diskSizeGB: number,
  ...
  Handlebars.registerHelper('recovery:estimateRestoreTime', function(
    diskSizeGB: number,
    isIncremental: boolean
  ) {
  ...
  Handlebars.registerHelper('recovery:snapshotSchedule', function(workloadType: string) {
    return JSON.stringify(getRecommendedSnapshotSchedule(workloadType as any));
  });
