...
export interface ReplicationPolicyConfig {
  name: string;
  vaultName: string;
  recoveryPointRetentionInHours?: number; // 1-72 hours
  appConsistentFrequencyInMinutes?: number; // 0-240 minutes
  crashConsistentFrequencyInMinutes?: number; // 5-240 minutes
}
 ...
 * ```
 */
export function replicationPolicy(config: ReplicationPolicyConfig): any {
  return {
    ...
    properties: {
      providerSpecificInput: {
        ...
        crashConsistentFrequencyInMinutes:
          config.crashConsistentFrequencyInMinutes ?? 5,
        appConsistentFrequencyInMinutes:
          config.appConsistentFrequencyInMinutes ?? 60,
 ...
 * ```
 */
export function enableVMReplication(config: VMReplicationConfig): any {
  const targetResourceGroup =
    config.targetResourceGroup || `${config.vmName}-dr-rg`;
...
export interface RecoveryPlanConfig {
  ...
  targetRegion: string;
  vmNames: string[];
  bootOrder?: number; // 1-3 (lower boots first)
}
 ...
 * ```
 */
export function recoveryPlan(config: RecoveryPlanConfig): any {
 ...
 */
export const RegionPairs: Record<string, string> = {
  eastus: "westus",
  eastus2: "centralus",
  westus: "eastus",
 ...
 */
export function getRecommendedTargetRegion(sourceRegion: string): string {
  return RegionPairs[sourceRegion.toLowerCase()] || "westus2";
}
 ...
 */
export function estimateRTO(vmCount: number, avgVMSize: number): number {
 ...
 */
export function estimateRPO(crashConsistentFrequencyMinutes: number): number {
 ...
 */
export function estimateReplicationBandwidth(
  vmSizeGB: number,
  dailyChangeRate: number = 0.1,
): number {
 ...
 */
export function validateReplicationPolicy(config: ReplicationPolicyConfig): {
  valid: boolean;
  errors: string[];
  warnings: string[];
} {
  ...
  if (config.recoveryPointRetentionInHours) {
    if (
      config.recoveryPointRetentionInHours < 1 ||
  ...
  if (config.crashConsistentFrequencyInMinutes) {
    if (
      ...
      errors.push(
        "Crash-consistent frequency must be between 5 and 240 minutes",
      );
    }
  }
  ...
  if (config.appConsistentFrequencyInMinutes !== undefined) {
    if (
      config.appConsistentFrequencyInMinutes < 0 ||
  ...
  // Warnings
  if (
    config.crashConsistentFrequencyInMinutes &&
    config.crashConsistentFrequencyInMinutes > 30
  ) {
    warnings.push(
      "Crash-consistent frequency > 30 minutes may result in higher RPO",
    );
  }

  if (
    config.recoveryPointRetentionInHours &&
    config.recoveryPointRetentionInHours < 24
  ) {
    warnings.push(
      "Recovery point retention < 24 hours is not recommended for production",
    );
  }
 ...
 */
export function siteRecoveryBestPractices(): string {
  return `
