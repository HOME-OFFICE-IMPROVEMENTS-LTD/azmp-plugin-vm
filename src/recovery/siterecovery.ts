...
export function validateReplicationPolicy(config: ReplicationPolicyConfig): {
  ...
  if (config.recoveryPointRetentionInHours) {
    if (config.recoveryPointRetentionInHours < 1 || config.recoveryPointRetentionInHours > 72) {
  ...
  if (config.crashConsistentFrequencyInMinutes) {
    if (config.crashConsistentFrequencyInMinutes < 5 || config.crashConsistentFrequencyInMinutes > 240) {
  ...
  if (config.appConsistentFrequencyInMinutes !== undefined) {
    if (config.appConsistentFrequencyInMinutes < 0 || config.appConsistentFrequencyInMinutes > 240) {
      errors.push('App-consistent frequency must be between 0 and 240 minutes');
    }
  }

  // Warnings
  if (config.crashConsistentFrequencyInMinutes && config.crashConsistentFrequencyInMinutes > 30) {
    warnings.push('Crash-consistent frequency > 30 minutes may result in higher RPO');
  }

  if (config.recoveryPointRetentionInHours && config.recoveryPointRetentionInHours < 24) {
