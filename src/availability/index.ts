...
export function registerAvailabilityHelpers() {
  ...
  Handlebars.registerHelper("availability:set", function (options: any) {
    const config: AvailabilitySetConfig = {
      ...
      platformFaultDomainCount:
        options.hash.faultDomains ?? options.hash.platformFaultDomainCount,
      platformUpdateDomainCount:
        options.hash.updateDomains ?? options.hash.platformUpdateDomainCount,
  ...
  Handlebars.registerHelper(
    "availability:recommendedFaultDomains",
    function (vmCount: number) {
      return recommendedFaultDomains(vmCount);
    },
  );

  Handlebars.registerHelper(
    "availability:recommendedUpdateDomains",
    function (vmCount: number) {
  ...
  Handlebars.registerHelper(
    "availability:proximityPlacementGroup",
    function (name: string, location?: string) {
  ...
  Handlebars.registerHelper(
    "availability:zoneRedundantDisk",
    function (diskName: string, zones?: string[]) {
  ...
  Handlebars.registerHelper(
    "availability:zoneRedundantIP",
    function (ipName: string, zones?: string[]) {
      const zoneArray = zones as ZoneArray | undefined;
  ...
  Handlebars.registerHelper(
    "availability:zoneSLA",
    function (vmCount: number, zoneCount: number) {
      return availabilityZoneSLA(vmCount, zoneCount);
    },
  );

  Handlebars.registerHelper(
    "availability:recommendZoneDistribution",
    function (vmCount: number) {
  ...
  Handlebars.registerHelper(
    ...
    function (vmssName: string, options: any) {
  ...
  Handlebars.registerHelper(
    "availability:cpuAutoscaleRules",
    function (vmssResourceId: string, options: any) {
      const scaleOutThreshold = options.hash.scaleOutThreshold ?? 75;
      const scaleInThreshold = options.hash.scaleInThreshold ?? 25;
      return JSON.stringify(
        cpuAutoscaleRules(vmssResourceId, scaleOutThreshold, scaleInThreshold),
        null,
        2,
      );
    },
  ...
  Handlebars.registerHelper(
    "availability:vmssHealthExtension",
    function (options: any) {
  ...
  Handlebars.registerHelper(
    "availability:rollingUpgradePolicy",
    function (options: any) {
      ...
      return JSON.stringify(
        rollingUpgradePolicy(maxBatchPercent, maxUnhealthyPercent, pauseTime),
        null,
        2,
      );
    },
  ...
  Handlebars.registerHelper(
    "availability:vmssSLA",
    function (zoneCount: number, instanceCount: number) {
