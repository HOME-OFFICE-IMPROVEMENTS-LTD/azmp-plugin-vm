...
export function azureDiskEncryption(
  ...
  const extensionType = platform === 'Windows' ? 'AzureDiskEncryption' : 'AzureDiskEncryptionForLinux';
  const publisher = platform === 'Windows' ? 'Microsoft.Azure.Security' : 'Microsoft.Azure.Security';
 ...
 */
export function customerManagedKey(keyVaultId: string, keyUrl: string): DiskEncryptionSetConfig {
  return {
 ...
 */
export function getEncryptionRecommendations(compliance: 'SOC2' | 'HIPAA' | 'ISO27001' | 'FedRAMP' | 'PCI-DSS' | 'GDPR') {
  const recommendations: Record<string, any> = {
    ...
    FedRAMP: {
      primary: 'AzureDiskEncryption',
      keyManagement: 'CustomerManaged',
      additionalFeatures: ['encryptionAtHost', 'doubleEncryption', 'keyRotation']
    },
    'PCI-DSS': {
      primary: 'AzureDiskEncryption',
      keyManagement: 'CustomerManaged',
 ...
 */
export function validateEncryptionConfig(config: any): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
  ...
  if (config.type === 'diskEncryption' && config.encryptionType?.includes('CustomerKey')) {
