...
export const securityTemplates: SecurityTemplateMetadata[] = [
  ...
  },
  {
    ...
    features: ['Server-Side Encryption', 'Customer-managed keys', 'Key Vault integration'],
  ...
  },
  {
    ...
    features: ['Secure Boot', 'vTPM', 'Boot Integrity Monitoring', 'Azure Security Center'],
  ...
  },
  {
    ...
    features: ['CMK Encryption', 'Trusted Launch Advanced', 'Encryption at Host', 'Logging'],
  ...
  },
  {
    ...
    features: ['ADE', 'CMK', 'Double Encryption', 'Trusted Launch Maximum', 'Attestation'],
    compliance: ['HIPAA'],
    priority: 'Must-Have'
  },
  {
    ...
    features: ['CMK Encryption', 'Trusted Launch Advanced', 'Access Controls', 'Audit Logs'],
  ...
  },
  {
    ...
    features: ['ADE', 'CMK', 'Double Encryption', 'Trusted Launch Maximum', 'Key Rotation', 'MFA'],
  ...
  },
  {
    ...
    features: ['ADE', 'CMK', 'Trusted Launch Advanced', 'Access Controls', 'Network Segmentation'],
  ...
  },
  {
    ...
    features: ['CMK Encryption', 'Data Residency', 'Encryption at Host', 'Access Logging'],
    compliance: ['GDPR'],
    priority: 'Should-Have'
  },
  {
  ...
  },
  {
    ...
    features: ['Dedicated Host', 'Network Isolation', 'Private Endpoints', 'CMK'],
...
export function createSecurityHelpers() {
  ...
  // Encryption Helpers
  helpers['security:encryption.ade'] = function(platform: 'Windows' | 'Linux', options: any) {
  ...
  helpers['security:encryption.cmk'] = function(keyVaultId: string, keyUrl: string) {
  ...
  helpers['security:encryption.recommendations'] = function(compliance: string) {
    return encryption.getEncryptionRecommendations(compliance as any);
  };

  // Trusted Launch Helpers
  helpers['security:trustedLaunch.secureBoot'] = function(enabled: boolean = true) {
  ...
  helpers['security:trustedLaunch.bootIntegrity'] = function(enabled: boolean = true) {
    return JSON.stringify(trustedlaunch.bootIntegrityMonitoring(enabled), null, 2);
  };

  helpers['security:trustedLaunch.guestAttestation'] = function(platform: 'Windows' | 'Linux') {
  ...
  helpers['security:trustedLaunch.config'] = function(level: 'standard' | 'advanced' | 'maximum', platform: 'Windows' | 'Linux') {
    return JSON.stringify(trustedlaunch.createTrustedLaunchConfig(level, platform), null, 2);
  };

  helpers['security:trustedLaunch.recommendations'] = function(compliance: string) {
  ...
  helpers['security:template.list'] = function(compliance?: string) {
    if (compliance) {
      return securityTemplates.filter(t => 
        t.compliance.some(c => c.toLowerCase().includes(compliance.toLowerCase()))
      );
    }
  ...
  helpers['security:template.listCompliance'] = function() {
    ...
    securityTemplates.forEach(t => t.compliance.forEach(c => complianceSet.add(c)));
    return Array.from(complianceSet).sort();
  };

  // Compliance Helpers
  helpers['security:compliance.getRequirements'] = function(framework: string) {
    const encRec = encryption.getEncryptionRecommendations(framework as any);
    const tlRec = trustedlaunch.getTrustedLaunchRecommendations(framework as any);
    ...
    return {
      ...
      templates: securityTemplates.filter(t => 
        t.compliance.some(c => c.toLowerCase().includes(framework.toLowerCase()))
      )
    };
  };
