...
export function secureBoot(enabled: boolean = true) {
  return {
    ...
    enabled,
    description:
      "Protects against boot kits and rootkits by validating boot signatures",
    requirements: [
      "Generation 2 VM",
...
export function validateTrustedLaunchConfig(
  ...
  if (
    config.uefiSettings?.secureBootEnabled &&
  ...
  if (config.guestAttestation && !config.bootIntegrityMonitoring) {
    warnings.push(
      "Guest Attestation is typically used with Boot Integrity Monitoring",
    );
  }
