...
describe('Extensions Module', () => {
  describe('Windows Extensions', () => {
    it('should create CustomScriptExtension', () => {
      ...
      expect(ext.settings?.fileUris).toEqual(['https://example.com/script.ps1']);
    });

    it('should create MonitoringAgentExtension', () => {
      const ext = windows.monitoringAgentExtension('workspace-id', 'workspace-key');
  ...
  describe('Linux Extensions', () => {
    ...
    it('should create RunCommandExtension', () => {
      ...
      expect(ext.protectedSettings?.commandToExecute).toBe('echo "Hello World"');
  ...
  describe('Cross-Platform Extensions', () => {
    ...
    it('should create KeyVaultExtension for Windows', () => {
      ...
      expect(ext.settings?.secretsManagementSettings.certificateStoreLocation).toBe('LocalMachine');
    });
    ...
    it('should create KeyVaultExtension for Linux', () => {
      ...
      expect(ext.settings?.secretsManagementSettings.pollingIntervalInS).toBe('3600');
    });
  });
  ...
  describe('Extensions Catalog', () => {
    ...
    it('should have 8 Windows extensions', () => {
      const windowsExts = extensionsCatalog.filter(ext => ext.category === 'windows');
      expect(windowsExts).toHaveLength(8);
    });

    it('should have 7 Linux extensions', () => {
      const linuxExts = extensionsCatalog.filter(ext => ext.category === 'linux');
      expect(linuxExts).toHaveLength(7);
    });

    it('should have 5 cross-platform extensions', () => {
      const crossExts = extensionsCatalog.filter(ext => ext.category === 'crossplatform');
      expect(crossExts).toHaveLength(5);
    });

    it('should have correct priorities', () => {
      const mustHave = extensionsCatalog.filter(ext => ext.priority === 'Must-Have');
      const shouldHave = extensionsCatalog.filter(ext => ext.priority === 'Should-Have');
      const niceToHave = extensionsCatalog.filter(ext => ext.priority === 'Nice-to-Have');
  ...
  describe('Extension Auto-Upgrade', () => {
    it('all extensions should have autoUpgradeMinorVersion enabled', () => {
      const winExt = windows.customScriptExtension({ commandToExecute: 'test' });
      const linuxExt = linux.customScriptExtension({ commandToExecute: 'test' });
