...
describe('Template Generation', () => {
  ...
  test('template with VM extensions should have valid structure', () => {
    ...
    const extensionResources = templateWithExtensions.resources.filter((resource: any) => 
    ...
    const vmResource = templateWithExtensions.resources.find((resource: any) => 
  ...
  test('template extension parameters should have proper defaults', () => {
    ...
    expect(extensionParameters.installCustomScriptExtension.defaultValue).toBe(false);
  ...
  test('template with availability set should have valid structure', () => {
    ...
    const availabilitySetResource = templateWithAvailabilitySet.resources.find((resource: any) => 
    ...
    const vmResource = templateWithAvailabilitySet.resources.find((resource: any) => 
      resource.type === 'Microsoft.Compute/virtualMachines'
    );
  ...
  test('template with backup configuration should have valid structure', () => {
    ...
    const vaultResource = templateWithBackup.resources.find((resource: any) => 
    ...
    const policyResource = templateWithBackup.resources.find((resource: any) => 
...
describe('CreateUiDefinition Template Validation', () => {
  ...
  test('UI definition should have wizard flow with correct steps', () => {
    ...
    const stepNames = mockUiDefinition.parameters.steps.map((step: any) => step.name);
  ...
  test('UI definition basics should include core VM parameters', () => {
    ...
    expect(mockBasics.find(c => c.name === 'vmSize')?.type).toBe('Microsoft.Compute.SizeSelector');
    expect(mockBasics.find(c => c.name === 'authenticationType')?.type).toBe('Microsoft.Common.DropDown');
  });
  ...
  test('networking step should include VNet and security controls', () => {
    ...
    expect(mockNetworkingStep.elements.find(e => e.name === 'virtualNetwork')?.type)
      .toBe('Microsoft.Network.VirtualNetworkCombo');
    expect(mockNetworkingStep.elements.find(e => e.name === 'publicIPAddress')?.type)
  ...
  test('extensions step should include monitoring and security options', () => {
    ...
    const mockExtensionsStep = {
      ...
      elements: [
        ...
        { 
          ...
          type: 'Microsoft.Common.Section',
          elements: [
            { name: 'installMonitoringExtension', type: 'Microsoft.Common.CheckBox' }
          ]
        },
        {
          ...
          type: 'Microsoft.Common.Section', 
          elements: [
            { name: 'installSecurityExtension', type: 'Microsoft.Common.CheckBox' },
            { name: 'enableManagedIdentity', type: 'Microsoft.Common.CheckBox' }
          ]
        },
        {
          ...
          type: 'Microsoft.Common.Section',
          elements: [
            { name: 'installCustomScriptExtension', type: 'Microsoft.Common.CheckBox' }
          ]
        }
      ]
    };
    ...
    const monitoringSection = mockExtensionsStep.elements.find(e => e.name === 'monitoringSection');
    expect(monitoringSection?.type).toBe('Microsoft.Common.Section');
    
    const securitySection = mockExtensionsStep.elements.find(e => e.name === 'securitySection');
  ...
  test('HA/DR step should include availability and backup options', () => {
    ...
    const availabilitySection = mockHadrStep.elements.find(e => e.name === 'availabilitySection');
    const availabilityChoice = (availabilitySection as any)?.elements?.find((e: any) => e.name === 'availabilityChoice');
    ...
    const optionValues = availabilityChoice?.constraints?.allowedValues?.map((opt: any) => opt.value);
  ...
  test('outputs should map to all template parameters', () => {
    const expectedOutputs = [
      // Basic outputs
      'vmName', 'adminUsername', 'authenticationType', 'adminPasswordOrKey', 'vmSize', 'location',
      // Networking outputs
      'virtualNetworkName', 'subnetName', 'publicIPName', 'allowSshRdp',
      // Extension outputs
      'installExtensions', 'installMonitoringExtension', 'installSecurityExtension', 'enableManagedIdentity',
      // HA/DR outputs
      'createAvailabilitySet', 'useAvailabilityZones', 'enableBackup', 'enableSnapshot'
    ];
    ...
    const mockOutputs = {
      ...
      installMonitoringExtension: '[steps("extensionsConfig").monitoringSection.installMonitoringExtension]',
      installSecurityExtension: '[steps("extensionsConfig").securitySection.installSecurityExtension]',
      enableManagedIdentity: '[steps("extensionsConfig").securitySection.enableManagedIdentity]',
    ...
    expect(mockOutputs.virtualNetworkName).toContain('steps("networkingConfig")');
  ...
  test('UI definition should handle conditional visibility correctly', () => {
    ...
    const conditionalVisibilityTests = [
      {
        ...
        condition: 'equals(steps("hadrConfig").availabilitySection.availabilityChoice, "availabilitySet")',
        description: 'Availability set config should only show when availability set is selected'
      },
      {
      ...
      },
      {
        ...
        condition: 'steps("extensionsConfig").customScriptSection.installCustomScriptExtension',
        description: 'Custom script config should only show when custom script extension is enabled'
      },
      {
        control: 'workspaceConfig',
        condition: 'steps("extensionsConfig").monitoringSection.installMonitoringExtension',
        description: 'Workspace config should only show when monitoring extension is enabled'
      }
    ];
    ...
    const mutualExclusionTest = {
      availabilitySet: 'equals(steps("hadrConfig").availabilitySection.availabilityChoice, "availabilitySet")',
      availabilityZones: 'equals(steps("hadrConfig").availabilitySection.availabilityChoice, "availabilityZones")',
  ...
  test('UI definition validation rules should be comprehensive', () => {
    const validationRules = {
      vmName: {
        ...
        message: 'VM name must be 1-64 characters, start and end with alphanumeric, can contain hyphens'
      },
      adminUsername: {
        regex: '^[a-zA-Z][a-zA-Z0-9]{2,19}$',
        message: 'Username must be 3-20 characters, start with letter, alphanumeric only'
      },
      sshPublicKey: {
      ...
      password: {
        ...
        message: 'Password must be at least 12 characters with uppercase, lowercase, and numbers'
      },
      customScriptUrl: {
