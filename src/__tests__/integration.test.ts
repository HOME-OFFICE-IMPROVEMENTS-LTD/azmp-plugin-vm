...
describe("VMSS Integration Tests", () => {
  test("VMSS Uniform mode template should have valid structure", () => {
    ...
    const vmssResource = vmssUniformTemplate.resources.find(
      (r: any) => r.type === "Microsoft.Compute/virtualMachineScaleSets",
    );
  ...
  test("VMSS Flexible mode template should have valid structure", () => {
    ...
    const vmssResource = vmssFlexibleTemplate.resources.find(
      (r: any) => r.type === "Microsoft.Compute/virtualMachineScaleSets",
    );
    expect(vmssResource).toBeDefined();
  ...
  test("VMSS with VMs should properly reference scale set", () => {
    ...
    const vmResource = vmssWithVMsTemplate.resources.find(
      (r: any) => r.type === "Microsoft.Compute/virtualMachines",
    );
...
describe("Auto-Scaling Integration Tests", () => {
  test("Auto-scale settings with metric-based rules should be valid", () => {
    const autoScaleTemplate = {
      ...
      outputs: {
        autoScaleSettingsId: {
          type: "string",
          value:
            "[resourceId('Microsoft.Insights/autoscaleSettings', variables('autoScaleSettingsName'))]",
        },
      },
    };
    ...
    const autoScaleResource = autoScaleTemplate.resources.find(
      (r: any) => r.type === "Microsoft.Insights/autoscaleSettings",
    );
    ...
    const scheduleAutoScaleTemplate = {
      ...
      variables: {
        autoScaleSettingsName:
          "[concat(parameters('vmssName'), '-schedule-autoscale')]",
      },
      resources: [
        {
          type: "Microsoft.Insights/autoscaleSettings",
  ...
  test("Auto-scale with predictive scaling should be valid", () => {
    const predictiveAutoScaleTemplate = {
      ...
      variables: {
        autoScaleSettingsName:
          "[concat(parameters('vmssName'), '-predictive-autoscale')]",
      },
      resources: [
        {
          type: "Microsoft.Insights/autoscaleSettings",
    ...
    expect(
      autoScaleResource.properties.predictiveAutoscalePolicy,
    ).toBeDefined();
    expect(
      autoScaleResource.properties.predictiveAutoscalePolicy.scaleMode,
    ).toBe("ForecastOnly");
  });
});
...
describe("Multi-Region Integration Tests", () => {
  test("Multi-region deployment with Traffic Manager should be valid", () => {
    const multiRegionTemplate = {
      ...
      resources: [
        ...
        },
        {
          ...
          properties: {
            targetResourceId:
              "[resourceId('Microsoft.Network/publicIPAddresses', 'primary-pip')]",
        ...
        },
        {
          ...
          properties: {
            targetResourceId:
              "[resourceId('Microsoft.Network/publicIPAddresses', 'secondary-pip')]",
            endpointStatus: "Enabled",
      ...
      outputs: {
        trafficManagerFqdn: {
          type: "string",
          value:
            "[reference(resourceId('Microsoft.Network/trafficManagerProfiles', variables('trafficManagerName'))).dnsConfig.fqdn]",
        },
      },
    };
    ...
    const tmResource = multiRegionTemplate.resources.find(
      (r: any) => r.type === "Microsoft.Network/trafficManagerProfiles",
    );
    ...
    // Verify endpoints
    const endpoints = multiRegionTemplate.resources.filter(
      (r: any) =>
            ...
            frontendEndpoints: [
              {
                ...
                properties: {
                  hostName:
                    "[concat(variables('frontDoorName'), '.azurefd.net')]",
  ...
  test("Multi-region deployment with paired regions should be valid", () => {
    const pairedRegionsTemplate = {
      ...
      outputs: {
        primaryVmssId: {
          type: "string",
          value:
            "[resourceId('Microsoft.Compute/virtualMachineScaleSets', variables('primaryVmssName'))]",
        },
        secondaryVmssId: {
          type: "string",
          value:
            "[resourceId('Microsoft.Compute/virtualMachineScaleSets', variables('secondaryVmssName'))]",
        },
        regionPairStatus: {
          type: "object",
          value: {
    ...
    const vmssResources = pairedRegionsTemplate.resources.filter(
      (r: any) => r.type === "Microsoft.Compute/virtualMachineScaleSets",
    );
...
describe("Load Balancing Integration Tests", () => {
  test("Standard Load Balancer with VMSS should be valid", () => {
    ...
    const lbResource = lbTemplate.resources.find(
      (r: any) => r.type === "Microsoft.Network/loadBalancers",
    );
  ...
  test("Application Gateway with VMSS backend should be valid", () => {
    ...
    const appGwResource = appGwTemplate.resources.find(
      (r: any) => r.type === "Microsoft.Network/applicationGateways",
    );
