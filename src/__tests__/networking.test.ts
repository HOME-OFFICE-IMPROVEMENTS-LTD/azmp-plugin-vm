...
describe('VmPlugin Networking Integration', () => {
  ...
  describe('Networking Helpers', () => {
    ...
    it('should provide net:bastion.featureEnabled helper', () => {
      ...
      expect(helpers['net:bastion.featureEnabled']('premium', 'tunneling')).toBe(true);
      expect(helpers['net:bastion.featureEnabled']('basic', 'tunneling')).toBe(false);
    });
    ...
    it('should provide net:common.subnetName helper', () => {
      ...
      expect(helpers['net:common.subnetName']('myapp', 'web')).toBe('subnet-myapp-web');
    });
    ...
    it('should provide net:common.peeringName helper', () => {
      ...
      expect(helpers['net:common.peeringName']('hub', 'spoke1')).toBe('peer-hub-to-spoke1');
    });
  });
  ...
  describe('Helper Count', () => {
    it('should have significantly more helpers than basic VM plugin', () => {
      ...
      const networkingHelpers = helperNames.filter(name => name.startsWith('net:'));
    ...
    it('should have helpers for all networking domains', () => {
      ...
      expect(helperNames.some(name => name.startsWith('net:vnet.'))).toBe(true);
      expect(helperNames.some(name => name.startsWith('net:subnet.'))).toBe(true);
      expect(helperNames.some(name => name.startsWith('net:nsg.'))).toBe(true);
      expect(helperNames.some(name => name.startsWith('net:lb.'))).toBe(true);
      expect(helperNames.some(name => name.startsWith('net:appgw.'))).toBe(true);
      expect(helperNames.some(name => name.startsWith('net:bastion.'))).toBe(true);
      expect(helperNames.some(name => name.startsWith('net:peering.'))).toBe(true);
      expect(helperNames.some(name => name.startsWith('net:common.'))).toBe(true);
    });
  });
});