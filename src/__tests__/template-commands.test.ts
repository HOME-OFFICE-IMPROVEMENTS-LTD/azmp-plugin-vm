...
describe('Template CLI Commands', () => {
  ...
  describe('template generate', () => {
    test('should register generate command', () => {
      ...
      const generateCommand = templateCommand!.commands.find(cmd => cmd.name() === 'generate');
      expect(generateCommand).toBeDefined();
      expect(generateCommand!.description()).toContain('Generate ARM templates');
    });
  ...
  describe('template validate', () => {
    test('should register validate command', () => {
      ...
      const validateCommand = templateCommand!.commands.find(cmd => cmd.name() === 'validate');
  ...
  describe('template test', () => {
    test('should register test command', () => {
      ...
      const testCommand = templateCommand!.commands.find(cmd => cmd.name() === 'test');
  ...
  describe('template deploy-guide', () => {
    test('should register deploy-guide command', () => {
      ...
      const deployGuideCommand = templateCommand!.commands.find(cmd => cmd.name() === 'deploy-guide');
      expect(deployGuideCommand).toBeDefined();
      expect(deployGuideCommand!.description()).toContain('Azure Portal deployment guide');
    });
  ...
  describe('Command integration', () => {
    test('should register all 4 template subcommands', () => {
      ...
      const subcommandNames = templateCommand!.commands.map(cmd => cmd.name());
    ...
    test('should properly nest template commands under parent command', () => {
      ...
      const templateCommand = vmCommand.commands.find(cmd => cmd.name() === 'template');
      expect(templateCommand).toBeDefined();
