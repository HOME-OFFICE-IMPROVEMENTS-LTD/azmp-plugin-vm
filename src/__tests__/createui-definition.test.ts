...
describe.skip("CreateUiDefinition Template - Wizard Structure (Future)", () => {
  ...
  describe("Schema Structure", () => {
    ...
    test("should configure wizard mode", () => {
      ...
      expect(uiDef.parameters.config.basics.description).toContain(
        "production-ready",
      );
    });
  });
  ...
  describe("Basics Parameters", () => {
    ...
    test("should validate vmName with proper regex", () => {
      const vmNameParam = uiDef.parameters.basics.find(
        (p: any) => p.name === "vmName",
      );
      expect(vmNameParam.constraints.required).toBe(true);
      expect(vmNameParam.constraints.regex).toBe("^[a-z0-9A-Z-]{3,64}$");
      expect(vmNameParam.constraints.validationMessage).toContain(
        "3-64 characters",
      );
    });

    test("should configure VM size selector", () => {
      const vmSizeParam = uiDef.parameters.basics.find(
        (p: any) => p.name === "vmSize",
      );
  ...
  describe("Monitoring Parameters", () => {
    test("should have enableMonitoring checkbox", () => {
      const monitoringStep = uiDef.parameters.steps.find(
        (s: any) => s.name === "monitoring",
      );
      const monitoringSection = monitoringStep.elements.find(
        (e: any) => e.name === "monitoringSection",
      );
      const enableParam = monitoringSection.elements.find(
        (e: any) => e.name === "enableMonitoring",
      );
    ...
    test("should configure Log Analytics workspace name", () => {
      const template = uiDef.parameters.steps
        .find((s: any) => s.name === "monitoring")
        .elements.find((e: any) => e.name === "monitoringSection")
        .elements.find((e: any) => e.name === "logAnalyticsWorkspaceName");

      expect(template.constraints.regex).toBe("^[a-zA-Z0-9-]{4,63}$");
      expect(template.constraints.validationMessage).toContain(
        "4-63 characters",
      );
    });
    ...
    test("should configure retention slider with correct range", () => {
      const retentionParam = uiDef.parameters.steps
        .find((s: any) => s.name === "monitoring")
        .elements.find((e: any) => e.name === "monitoringSection")
        .elements.find((e: any) => e.name === "logAnalyticsRetentionDays");
    ...
    test("should configure metric alert thresholds", () => {
      const alertsSection = uiDef.parameters.steps
        .find((s: any) => s.name === "monitoring")
        .elements.find((e: any) => e.name === "alertsSection");

      const cpuThreshold = alertsSection.elements.find(
        (e: any) => e.name === "cpuAlertThreshold",
      );
      const memoryThreshold = alertsSection.elements.find(
        (e: any) => e.name === "memoryAlertThreshold",
      );
      const diskThreshold = alertsSection.elements.find(
        (e: any) => e.name === "diskAlertThreshold",
      );
    ...
    test("should configure workbook types dropdown", () => {
      const workbooksSection = uiDef.parameters.steps
        .find((s: any) => s.name === "monitoring")
        .elements.find((e: any) => e.name === "workbooksSection");

      const workbookTypes = workbooksSection.elements.find(
        (e: any) => e.name === "workbookTypes",
      );
      expect(workbookTypes.multiselect).toBe(true);
      expect(workbookTypes.constraints.allowedValues).toHaveLength(4);

      const values = workbookTypes.constraints.allowedValues.map(
        (v: any) => v.value,
      );
  ...
  describe("Cost & Performance Parameters", () => {
    test("should have cost optimization checkbox", () => {
      const costSection = uiDef.parameters.steps
        .find((s: any) => s.name === "costPerformance")
        .elements.find((e: any) => e.name === "costSection");

      const enableParam = costSection.elements.find(
        (e: any) => e.name === "enableCostOptimization",
      );
      expect(enableParam.type).toBe("Microsoft.Common.CheckBox");
      expect(enableParam.defaultValue).toBe(true);
    });
    ...
    test("should validate budget limit", () => {
      const budgetParam = uiDef.parameters.steps
        .find((s: any) => s.name === "costPerformance")
        .elements.find((e: any) => e.name === "costSection")
        .elements.find((e: any) => e.name === "monthlyBudgetLimit");
    ...
    test("should configure performance profile dropdown", () => {
      const perfSection = uiDef.parameters.steps
        .find((s: any) => s.name === "costPerformance")
        .elements.find((e: any) => e.name === "performanceSection");

      const profileParam = perfSection.elements.find(
        (e: any) => e.name === "performanceProfile",
      );
      expect(profileParam.type).toBe("Microsoft.Common.DropDown");
      expect(profileParam.constraints.allowedValues).toHaveLength(3);

      const values = profileParam.constraints.allowedValues.map(
        (v: any) => v.value,
      );
    ...
    test("should configure autoscale parameters", () => {
      ...
      const minInstances = perfSection.elements.find(
        (e: any) => e.name === "autoscaleMinInstances",
      );
      const maxInstances = perfSection.elements.find(
        (e: any) => e.name === "autoscaleMaxInstances",
      );
      const cpuThreshold = perfSection.elements.find(
        (e: any) => e.name === "autoscaleCpuThreshold",
      );
  ...
  describe("High Availability Parameters", () => {
    test("should have HA enable checkbox", () => {
      const haSection = uiDef.parameters.steps
        .find((s: any) => s.name === "highAvailability")
        .elements.find((e: any) => e.name === "haSection");

      const enableParam = haSection.elements.find(
        (e: any) => e.name === "enableHighAvailability",
      );
      expect(enableParam.type).toBe("Microsoft.Common.CheckBox");
    ...
    test("should configure availability option dropdown", () => {
      ...
      const availOption = haSection.elements.find(
        (e: any) => e.name === "availabilityOption",
      );
      expect(availOption.type).toBe("Microsoft.Common.DropDown");
      expect(availOption.constraints.allowedValues).toHaveLength(4);

      const values = availOption.constraints.allowedValues.map(
        (v: any) => v.value,
      );
    ...
    test("should configure availability zones multiselect", () => {
      ...
      const zonesParam = haSection.elements.find(
        (e: any) => e.name === "availabilityZones",
      );
    ...
    test("should configure availability set fault/update domains", () => {
      ...
      const faultDomains = haSection.elements.find(
        (e: any) => e.name === "availabilitySetFaultDomains",
      );
      const updateDomains = haSection.elements.find(
        (e: any) => e.name === "availabilitySetUpdateDomains",
      );
    ...
    test("should validate VMSS instance count", () => {
      ...
      const vmssCount = haSection.elements.find(
        (e: any) => e.name === "vmssInstanceCount",
      );
  ...
  describe("Disaster Recovery Parameters", () => {
    test("should have DR enable checkbox", () => {
      const drSection = uiDef.parameters.steps
        .find((s: any) => s.name === "disasterRecovery")
        .elements.find((e: any) => e.name === "drSection");

      const enableParam = drSection.elements.find(
        (e: any) => e.name === "enableDisasterRecovery",
      );
      expect(enableParam.type).toBe("Microsoft.Common.CheckBox");
      expect(enableParam.defaultValue).toBe(false);
    });
    ...
    test("should configure backup policy dropdown", () => {
      ...
      const policyParam = drSection.elements.find(
        (e: any) => e.name === "backupPolicyType",
      );
      expect(policyParam.type).toBe("Microsoft.Common.DropDown");

      const values = policyParam.constraints.allowedValues.map(
        (v: any) => v.value,
      );
    ...
    test("should configure backup retention slider", () => {
      ...
      const retentionParam = drSection.elements.find(
        (e: any) => e.name === "backupRetentionDays",
      );
      expect(retentionParam.type).toBe("Microsoft.Common.Slider");
    ...
    test("should configure backup frequency dropdown", () => {
      ...
      const frequencyParam = drSection.elements.find(
        (e: any) => e.name === "backupFrequency",
      );
      const values = frequencyParam.constraints.allowedValues.map(
        (v: any) => v.value,
      );
    ...
    test("should validate backup time format", () => {
      ...
      const timeParam = drSection.elements.find(
        (e: any) => e.name === "backupTime",
      );
      expect(timeParam.constraints.regex).toBe(
    ...
    test("should configure site recovery parameters", () => {
      ...
      const siteRecoveryParam = drSection.elements.find(
        (e: any) => e.name === "enableSiteRecovery",
      );
      const recoveryRegionParam = drSection.elements.find(
        (e: any) => e.name === "recoveryRegion",
      );
  ...
  describe("Conditional Visibility", () => {
    test("should hide monitoring fields when monitoring disabled", () => {
      ...
      const workspaceName = monitoringSection.elements.find(
        (e: any) => e.name === "logAnalyticsWorkspaceName",
      );

      expect(workspaceName.visible).toContain(
        "steps('monitoring').monitoringSection.enableMonitoring",
      );
    });
    ...
    test("should hide availability zones when option not selected", () => {
      ...
      expect(zonesParam.visible).toContain(
        "equals(steps('highAvailability').haSection.availabilityOption, 'AvailabilityZones')",
      );
    });
  ...
  describe("Output Mappings", () => {
    ...
    test("should map monitoring parameters to outputs", () => {
      ...
      expect(outputs.enableMonitoring).toBe(
        "[steps('monitoring').monitoringSection.enableMonitoring]",
      );
      expect(outputs.logAnalyticsWorkspaceName).toBe(
        "[steps('monitoring').monitoringSection.logAnalyticsWorkspaceName]",
      );
      expect(outputs.logAnalyticsRetentionDays).toBe(
        "[steps('monitoring').monitoringSection.logAnalyticsRetentionDays]",
      );
      expect(outputs.enableApplicationInsights).toBe(
        "[steps('monitoring').monitoringSection.enableApplicationInsights]",
      );
    });
    ...
    test("should map cost/performance parameters to outputs", () => {
      ...
      expect(outputs.enableCostOptimization).toBe(
        "[steps('costPerformance').costSection.enableCostOptimization]",
      );
      expect(outputs.monthlyBudgetLimit).toBe(
        "[int(steps('costPerformance').costSection.monthlyBudgetLimit)]",
      );
      expect(outputs.enablePerformanceOptimization).toBe(
        "[steps('costPerformance').performanceSection.enablePerformanceOptimization]",
      );
      expect(outputs.performanceProfile).toBe(
        "[steps('costPerformance').performanceSection.performanceProfile]",
      );
    });
    ...
    test("should map HA parameters to outputs", () => {
      ...
      expect(outputs.enableHighAvailability).toBe(
        "[steps('highAvailability').haSection.enableHighAvailability]",
      );
      expect(outputs.availabilityOption).toBe(
        "[steps('highAvailability').haSection.availabilityOption]",
      );
      expect(outputs.availabilityZones).toBe(
        "[steps('highAvailability').haSection.availabilityZones]",
      );
    });
    ...
    test("should map DR parameters to outputs", () => {
      ...
      expect(outputs.enableDisasterRecovery).toBe(
        "[steps('disasterRecovery').drSection.enableDisasterRecovery]",
      );
      expect(outputs.enableBackup).toBe(
        "[steps('disasterRecovery').drSection.enableBackup]",
      );
      expect(outputs.backupPolicyType).toBe(
        "[steps('disasterRecovery').drSection.backupPolicyType]",
      );
      expect(outputs.backupRetentionDays).toBe(
        "[steps('disasterRecovery').drSection.backupRetentionDays]",
      );
    });
    ...
    test("should handle email recipients array transformation", () => {
      ...
      expect(outputs.alertEmailRecipients).toContain(
        "steps('monitoring').alertsSection.alertEmailRecipients",
      );
    });
  });
  ...
  describe("Tooltips & Descriptions", () => {
    ...
    test("should explain performance profiles in labels", () => {
      ...
      const labels = profileParam.constraints.allowedValues.map(
        (v: any) => v.label,
      );
