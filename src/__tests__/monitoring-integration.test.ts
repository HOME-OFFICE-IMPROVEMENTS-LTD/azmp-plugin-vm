...
describe('Monitoring Integration Tests', () => {
  ...
  describe('Complete Monitoring Stack', () => {
    it('should generate complete monitoring infrastructure with workspace, diagnostics, metrics, and alerts', () => {
      const workspaceHelper = Handlebars.helpers['monitor:logAnalyticsWorkspace'];
      const diagnosticsHelper = Handlebars.helpers['monitor:diagnosticSettings'];
      ...
      expect(workspaceJson.type).toBe('Microsoft.OperationalInsights/workspaces');
      ...
      const actionGroup = (actionGroupHelper as any).call(null, {
        name: 'ag-ops-team',
        shortName: 'OpsTeam',
        emailReceivers: JSON.stringify([{ name: 'ops', emailAddress: 'ops@example.com' }])
      });
      ...
      const diagnostics = (diagnosticsHelper as any).call(null, {
        hash: {
          name: 'diag-vm-001',
          targetResourceId: '/subscriptions/sub-id/resourceGroups/rg/providers/Microsoft.Compute/virtualMachines/vm-001',
          workspaceId: '/subscriptions/sub-id/resourceGroups/rg/providers/Microsoft.OperationalInsights/workspaces/law-monitoring',
      ...
      expect(diagnosticsJson.type).toBe('Microsoft.Insights/diagnosticSettings');
      expect(diagnosticsJson.properties.workspaceId).toContain('law-monitoring');
      ...
      expect(metricsJson.properties.criteria.allOf[0].metricName).toBe('Percentage CPU');
      ...
      const alert = (alertHelper as any).call(null, {
        ...
        severity: 2,
        scopes: JSON.stringify(['/subscriptions/sub-id/resourceGroups/rg/providers/Microsoft.Compute/virtualMachines/vm-001']),
        ...
        actionGroupIds: JSON.stringify(['/subscriptions/sub-id/resourceGroups/rg/providers/microsoft.insights/actionGroups/ag-ops-team'])
      });
  ...
  describe('VMSS Monitoring Stack', () => {
    it('should generate VMSS monitoring with autoscale metrics, alerts, and dashboard', () => {
      ...
      const vmssResourceId = '/subscriptions/sub-id/resourceGroups/rg/providers/Microsoft.Compute/virtualMachineScaleSets/vmss-web';
      ...
      const metrics = (metricsHelper as any).call(null, {
        hash: {
          ...
          metrics: JSON.stringify(['Percentage CPU', 'Network In Total', 'Network Out Total']),
          aggregation: 'Average',
          frequency: 'PT1M'
        }
      });
      ...
      const alertJson = JSON.parse(alert);
      expect(alertJson.type).toBe('Microsoft.Insights/metricAlerts');
      expect(alertJson.properties.criteria['odata.type']).toBe('Microsoft.Azure.Monitor.MultipleResourceMultipleMetricCriteria');
      ...
      expect(dashboardJson.properties.lenses[0].parts.length).toBeGreaterThan(0);
      
      // Verify dashboard includes instance count tile
      const instanceCountTile = dashboardJson.properties.lenses[0].parts.find((part: { metadata: { settings: { content: { options: { chart: { title: string } } } } } }) => 
        part.metadata?.settings?.content?.options?.chart?.title?.includes('Instance Count')
      );
  ...
  describe('Multi-Region Health Monitoring', () => {
    it('should generate multi-region monitoring with health checks and regional dashboards', () => {
      const workspaceHelper = Handlebars.helpers['monitor:logAnalyticsWorkspace'];
      const appInsightsHelper = Handlebars.helpers['monitor:applicationInsights'];
      const dashboardHelper = Handlebars.helpers['dashboard:multiRegionHealth'];
      const activityLogAlertHelper = Handlebars.helpers['alert:activityLogAlert'];
      ...
      const appInsightsEastUS = (appInsightsHelper as any).call(null, {
        hash: {
          ...
          location: 'eastus',
          applicationType: 'web',
          workspaceResourceId: '/subscriptions/sub-id/resourceGroups/rg/providers/Microsoft.OperationalInsights/workspaces/law-global-monitoring'
        }
      });
      ...
      const dashboard = (dashboardHelper as any).call(null, {
        ...
        regions: [
          {
            name: 'East US',
            vmResourceIds: ['/subscriptions/sub-id/resourceGroups/rg-eastus/providers/Microsoft.Compute/virtualMachines/vm-east-001'],
            loadBalancerResourceId: '/subscriptions/sub-id/resourceGroups/rg-eastus/providers/Microsoft.Network/loadBalancers/lb-east'
          },
          {
            name: 'West US',
            vmResourceIds: ['/subscriptions/sub-id/resourceGroups/rg-westus/providers/Microsoft.Compute/virtualMachines/vm-west-001'],
            loadBalancerResourceId: '/subscriptions/sub-id/resourceGroups/rg-westus/providers/Microsoft.Network/loadBalancers/lb-west'
          }
        ],
      ...
      const dashboardJson = JSON.parse(dashboard);
      expect(dashboardJson.type).toBe('Microsoft.Portal/dashboards');
      expect(dashboardJson.properties.lenses[0].parts.length).toBeGreaterThanOrEqual(2);
      ...
      expect(alertJson.properties.condition.allOf[0].equals).toBe('ServiceHealth');
    });
  });
  ...
  describe('Load Balancer Monitoring', () => {
    it('should generate load balancer monitoring with health probes, throughput metrics, and alerts', () => {
      ...
      const metricsHelper = Handlebars.helpers['monitor:metrics'];
      const alertHelper = Handlebars.helpers['alert:metricAlert'];
      const dashboardHelper = Handlebars.helpers['dashboard:loadBalancerPerformance'];

      const lbResourceId = '/subscriptions/sub-id/resourceGroups/rg/providers/Microsoft.Network/loadBalancers/lb-prod';
      ...
      const diagnostics = (diagnosticsHelper as any).call(null, {
        hash: {
          ...
          workspaceId: '/subscriptions/sub-id/resourceGroups/rg/providers/Microsoft.OperationalInsights/workspaces/law-monitoring',
          metrics: JSON.stringify([{ category: 'AllMetrics', enabled: true }]),
          logs: JSON.stringify([{ category: 'LoadBalancerProbeHealthStatus', enabled: true }])
        }
      });
      expect(diagnostics).toBeDefined();
      ...
      const metrics = (metricsHelper as any).call(null, {
        hash: {
          targetResourceId: lbResourceId,
          metrics: JSON.stringify(['VipAvailability', 'DipAvailability', 'ByteCount', 'PacketCount', 'SYNCount']),
          aggregation: 'Average',
          frequency: 'PT1M'
        }
      });
      expect(metrics).toBeDefined();
      const metricsJson = JSON.parse(metrics);
      expect(metricsJson.properties.criteria.allOf[0].metricName).toBe('VipAvailability');
      ...
      const healthProbeTiles = dashboardJson.properties.lenses[0].parts.filter((part: { metadata: { settings: { content: { options: { chart: { title: string } } } } } }) => 
        part.metadata?.settings?.content?.options?.chart?.title?.includes('Health Probe') ||
        part.metadata?.settings?.content?.options?.chart?.title?.includes('SNAT Port')
      );
  ...
  describe('Log Analytics Integration', () => {
    it('should generate Log Analytics workspace with data collection rules and log alerts', () => {
      const workspaceHelper = Handlebars.helpers['monitor:logAnalyticsWorkspace'];
      const dataCollectionHelper = Handlebars.helpers['monitor:dataCollectionRule'];
      ...
      const dataCollection = (dataCollectionHelper as any).call(null, {
        hash: {
          ...
          destinations: JSON.stringify({
            logAnalytics: [{
              workspaceResourceId: '/subscriptions/sub-id/resourceGroups/rg/providers/Microsoft.OperationalInsights/workspaces/law-security',
      ...
      expect(dataCollectionJson.type).toBe('Microsoft.Insights/dataCollectionRules');
      expect(dataCollectionJson.properties.dataSources.performanceCounters[0].samplingFrequencyInSeconds).toBe(60);
      ...
      const alert = (logAlertHelper as any).call(null, {
        ...
        scopes: JSON.stringify(['/subscriptions/sub-id/resourceGroups/rg/providers/Microsoft.OperationalInsights/workspaces/law-security']),
        evaluationFrequency: 'PT5M',
        windowSize: 'PT15M',
        severity: 2,
      ...
      expect(alertJson.properties.criteria.allOf[0].query).toContain('SecurityEvent');
  ...
  describe('Application Insights Integration', () => {
    it('should generate Application Insights with custom metrics and availability tests', () => {
      ...
      expect(customMetricJson.metricDefinition.namespace).toBe('CustomApp/Orders');
      ...
      const alert = (alertHelper as any).call(null, {
        ...
        severity: 3,
        scopes: JSON.stringify(['/subscriptions/sub-id/resourceGroups/rg/providers/Microsoft.Insights/components/ai-webapp']),
        evaluationFrequency: 'PT5M',
        windowSize: 'PT15M',
        criteria: JSON.stringify([{
      ...
      expect(alert).toBeDefined();
      const alertJson = JSON.parse(alert);
      expect(alertJson.properties.criteria.allOf[0].metricName).toBe('OrdersProcessed');
    });
  });
  ...
  describe('Cost Monitoring', () => {
    it('should generate cost analysis dashboard with budget alerts', () => {
      ...
      const costTiles = dashboardJson.properties.lenses[0].parts.filter((part: { metadata: { type: string } }) => 
        part.metadata?.type?.includes('CostManagement') || part.metadata?.type?.includes('Budget')
      );
  ...
  describe('Security Monitoring Workbook', () => {
    it('should generate security posture workbook with Defender for Cloud integration', () => {
      ...
      const workbook = (workbookHelper as any).call(null, {
        ...
        subscriptionId: 'sub-id-123',
        workspaceResourceId: '/subscriptions/sub-id-123/resourceGroups/rg-security/providers/Microsoft.OperationalInsights/workspaces/law-security-hub',
      ...
      const securityItems = serializedData.items.filter((item: { content: { query: string } }) => 
      ...
      const alert = (logAlertHelper as any).call(null, {
        ...
        windowSize: 'PT1H',
        severity: 0,
        query: 'securityresources | where type == "microsoft.security/assessments" | where properties.status.code != "Healthy" and properties.metadata.severity == "High" | summarize count()',
        threshold: 1,
      ...
      expect(alertJson.properties.criteria.allOf[0].query).toContain('securityresources');
    });
  });
});
