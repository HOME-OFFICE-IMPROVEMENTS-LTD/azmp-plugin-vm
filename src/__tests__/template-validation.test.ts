...
describe("Template Validation System", () => {
  describe("ViewDefinition Template", () => {
    ...
    beforeAll(() => {
      const viewDefPath = path.join(
        __dirname,
        "../../templates/viewDefinition.json.hbs",
      );
    ...
    test("should have correct schema and content version", () => {
      expect(viewDef.$schema).toBe(
        "https://schema.management.azure.com/schemas/viewdefinition/0.0.1-preview/ViewDefinition.json#",
      );
    ...
    test("should have monitoring view with Log Analytics and Application Insights", () => {
      const monitoringView = viewDef.views.find(
        (v: any) => v.properties.displayName === "Monitoring",
      );
      expect(monitoringView).toBeDefined();
      expect(monitoringView.properties.commands).toBeDefined();
      expect(monitoringView.properties.commands.length).toBeGreaterThanOrEqual(
        2,
      );

      const commandNames = monitoringView.properties.commands.map(
        (c: any) => c.displayName,
      );
    ...
    test("should have cost management view with budget tracking", () => {
      const costView = viewDef.views.find(
        (v: any) => v.properties.displayName === "Cost Management",
      );
      expect(costView).toBeDefined();
      expect(costView.properties.resources).toBeDefined();

      const resourceTypes = costView.properties.resources.map(
        (r: any) => r.type,
      );
    ...
    test("should have resilience view with HA and DR resources", () => {
      const resilienceView = viewDef.views.find(
        (v: any) => v.properties.displayName === "Resilience",
      );
      expect(resilienceView).toBeDefined();
      expect(resilienceView.properties.resources).toBeDefined();

      const resourceTypes = resilienceView.properties.resources.map(
        (r: any) => r.type,
      );
    ...
    test("should have performance metrics view with charts", () => {
      const metricsView = viewDef.views.find((v: any) => v.kind === "Metrics");
      ...
      const chartNames = metricsView.properties.charts.map(
        (c: any) => c.displayName,
      );
    ...
    test("should have outputs view with deployment information", () => {
      const outputsView = viewDef.views.find(
        (v: any) => v.properties.displayName === "Outputs",
      );
