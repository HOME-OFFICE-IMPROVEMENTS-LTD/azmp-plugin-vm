 ...
 */
export enum DiskStorageAccountType {
 ...
 */
export enum PremiumSSDPerformanceTier {
  P1 = 'P1',
  P2 = 'P2',
  P3 = 'P3',
  P4 = 'P4',
 ...
 */
export class DiskTypeManager {
  private config: DiskConfiguration;
  
  /**
   * Disk type metadata registry
   */
  private static readonly DISK_TYPES: Record<DiskStorageAccountType, DiskTypeInfo> = {
 ...
 */
export function createDiskConfiguration(options: {
  osDiskType: string;
  osDiskSize?: number;
  ...
  const config: DiskConfiguration = {
    osDiskType: options.osDiskType as DiskStorageAccountType
  };
  ...
  if (options.osDiskCaching) {
    config.osDiskCaching = options.osDiskCaching as DiskCaching;
  }
  
  if (options.osDiskPerformanceTier) {
    config.osDiskPerformanceTier = options.osDiskPerformanceTier as PremiumSSDPerformanceTier;
  }
  
  if (options.dataDiskType) {
    config.dataDiskType = options.dataDiskType as DiskStorageAccountType;
  }
  
  if (options.dataDiskCount && options.dataDiskCount > 0) {
    config.dataDisks = [];
    const diskSize = options.dataDiskSize || 128;
    const diskType = (options.dataDiskType as DiskStorageAccountType) || DiskStorageAccountType.StandardSSD;
 ...
 */
export function generateDiskTemplate(config: DiskConfiguration): any {
