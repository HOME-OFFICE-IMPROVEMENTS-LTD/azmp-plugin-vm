 ...
 */
export class CertificationTestRunner {
  private config: CertificationTestConfig;
  ...
  async runVHDValidation(): Promise<VHDValidation> {
    ...
    try {
      const validation: VHDValidation = {
        isValid: true,
        format: 'VHD',
        sizeGB: 0,
        isFixedSize: false,
   ...
   */
  async runGeneralizationCheck(): Promise<boolean> {
   ...
   */
  async runConfigurationValidation(): Promise<boolean> {
   ...
   */
  async runComplianceChecks(): Promise<boolean> {
   ...
   */
  private generateResults(): CertificationResults {
...
export async function runBatchTests(
  ...
  const results = new Map<string, CertificationResults>();
 ...
 */
export async function quickValidate(vhdPath: string): Promise<VHDValidation> {
  const runner = new CertificationTestRunner({
    vhdPath,
