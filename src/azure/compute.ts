...
import { ComputeManagementClient, VirtualMachineSize, VirtualMachineImageResource } from '@azure/arm-compute';
...
export class ComputeHelper {
   ...
   */
  public async getVmSize(location: string, sizeName: string): Promise<VmSizeInfo | null> {
    try {
  ...
  public async listVmImagePublishers(location: string): Promise<string[]> {
    try {
      ...
      return publishers.map(p => p.name || 'Unknown').filter(name => name !== 'Unknown');
    } catch (error) {
   ...
   */
  public async listVmImageOffers(location: string, publisherName: string): Promise<string[]> {
    try {
      const offers = await this.client.virtualMachineImages.listOffers(location, publisherName);
      return offers.map(o => o.name || 'Unknown').filter(name => name !== 'Unknown');
    } catch (error) {
   ...
   */
  public async listVmImageSkus(location: string, publisherName: string, offer: string): Promise<string[]> {
    try {
      const skus = await this.client.virtualMachineImages.listSkus(location, publisherName, offer);
      return skus.map(s => s.name || 'Unknown').filter(name => name !== 'Unknown');
    } catch (error) {
  ...
  public async listVmImageVersions(
    ...
    try {
      ...
      return versions.map(v => v.name || 'Unknown').filter(name => name !== 'Unknown');
    } catch (error) {
