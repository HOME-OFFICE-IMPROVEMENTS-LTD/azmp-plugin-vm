...
export class VmPlugin implements IPlugin {
  ...
  private async loadJsonInput(input: string | undefined, label: string): Promise<any> {
  ...
  registerCommands(program: Command): void {
      ...
      .action(async (options) => {
        if (this.context) {
          try {
            ...
            if (options.category) {
              const categoryInput = (options.category as string).trim().toLowerCase() as WorkbookCategory;
              ...
              if (!validCategories.includes(categoryInput)) {
                ...
                templates = WorkbookTemplateManager.getTemplatesByCategory(categoryInput);
              }
            }
            ...
            if (options.tags && templates.length > 0) {
              ...
              templates = templates.filter((template) =>
                normalizedTags.some((tag: string) => template.tags.includes(tag))
              );
            }
    ...
    vmCommand
      ...
      .option('--notify <notification>', 'Notification rule in format threshold:email1;email2 (repeatable)', (value, previous: string[]) => {
        const list = previous || [];
        list.push(value);
        return list;
      })
      .option('--action-group <group>', 'Action group resource ID (repeatable)', (value, previous: string[]) => {
        const list = previous || [];
        list.push(value);
        return list;
      })
      .action(async (options) => {
        ...
        try {
          ...
          const { createBudgetDefinition, createCostAlertTemplate } = await import('./cost');
    ...
    vmCommand
      ...
      .option('--notify <email>', 'Notification email (repeatable)', (value: string, previous: string[] = []) => {
        previous.push(value);
        return previous;
      })
    ...
    vmCommand
      ...
      .action(async (options) => {
        ...
        try {
          ...
          if (options.arm) {
            outputPayload = MonitoringAlertEngine.toMetricAlertResource(definition);
          }
    ...
    vmCommand
      ...
      .action(async (options) => {
        if (!this.context) return;
        try {
          const thresholdMb = Number(options.thresholdMb || options.thresholdmb || options['threshold-mb']);
