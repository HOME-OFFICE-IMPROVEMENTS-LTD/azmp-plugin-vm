...
export const RESERVED_SUBNET_NAMES = {
  ...
  AzureFirewallManagementSubnet: {
    ...
    minPrefix: 26,
    description:
      "Azure Firewall Management requires exact name and minimum /26",
  },
} as const;
 ...
 */
export function validateSubnetName(name: string): {
  valid: boolean;
  error?: string;
} {
  ...
  if (!validNameRegex.test(name)) {
    return {
      valid: false,
      error:
        "Subnet name can only contain alphanumeric, underscore, hyphen, and period",
    };
  }
...
export function validateReservedSubnet(
  ...
  const reserved = Object.values(RESERVED_SUBNET_NAMES).find(
    (r) => r.name === name,
  );
...
export function createSubnetConfig(
  ...
  return {
    ...
    privateLinkServiceNetworkPolicies:
      options?.privateLinkServiceNetworkPolicies,
  };
}
