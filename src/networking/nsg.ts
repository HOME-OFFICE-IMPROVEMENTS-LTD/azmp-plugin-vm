...
export const NSG_TEMPLATES = {
  ...
  'web-server': {
    ...
    rules: ['allow-http', 'allow-https'] as NsgRuleKey[],
  },
  ...
  'database-server': {
    ...
    rules: ['allow-sql', 'allow-mysql', 'allow-postgresql'] as NsgRuleKey[],
  },
  ...
  'windows-server': {
    ...
    rules: ['allow-rdp', 'allow-winrm-http', 'allow-winrm-https'] as NsgRuleKey[],
  },
  ...
  'app-server': {
    ...
    rules: ['allow-http', 'allow-https', 'allow-app-8080', 'allow-app-8443'] as NsgRuleKey[],
  },
  ...
  'container-host': {
    ...
    rules: ['allow-docker', 'allow-docker-tls', 'allow-k8s-api'] as NsgRuleKey[],
  },
  ...
  'bastion': {
    ...
    rules: ['allow-ssh', 'allow-rdp'] as NsgRuleKey[],
  },
  ...
  'locked-down': {
    ...
    rules: ['deny-all-inbound', 'deny-all-outbound'] as NsgRuleKey[],
  },
} as const;
 ...
 * Get NSG rule by key
 */
export function getNsgRule(key: NsgRuleKey): typeof NSG_RULES[NsgRuleKey] | undefined {
  return NSG_RULES[key];
}

/**
 * Get all NSG rules
 */
export function getAllNsgRules(): Array<{ key: NsgRuleKey; rule: typeof NSG_RULES[NsgRuleKey] }> {
  return Object.entries(NSG_RULES).map(([key, rule]) => ({
    key: key as NsgRuleKey,
    rule,
  }));
}
 ...
 */
export function getNsgRulesByDirection(
  direction: 'Inbound' | 'Outbound'
): Array<{ key: NsgRuleKey; rule: typeof NSG_RULES[NsgRuleKey] }> {
 ...
 */
export function getNsgRulesByProtocol(
 ...
 */
export function getNsgTemplate(key: NsgTemplateKey): typeof NSG_TEMPLATES[NsgTemplateKey] | undefined {
  return NSG_TEMPLATES[key];
}

/**
 * Get all NSG templates
 */
export function getAllNsgTemplates(): Array<{ key: NsgTemplateKey; template: typeof NSG_TEMPLATES[NsgTemplateKey] }> {
  return Object.entries(NSG_TEMPLATES).map(([key, template]) => ({
    key: key as NsgTemplateKey,
    template,
  }));
}
 ...
 */
export function validateNsgPriority(priority: number): { valid: boolean; error?: string } {
  if (priority < 100 || priority > 4096) {
    return { valid: false, error: 'NSG rule priority must be between 100 and 4096' };
  }
  return { valid: true };
}
 ...
 * Validate port range
 */
export function validatePortRange(portRange: string): { valid: boolean; error?: string } {
  // Allow * for any port
  if (portRange === '*') return { valid: true };

  // Allow single port (e.g., "80")
  const singlePort = /^\d+$/;
  if (singlePort.test(portRange)) {
    const port = parseInt(portRange, 10);
    if (port >= 0 && port <= 65535) return { valid: true };
  ...
  return { valid: false, error: 'Invalid port format. Use * or number or range (e.g., 80-443)' };
}

/**
 * Get service tag description
 */
export function getServiceTagDescription(tag: keyof typeof SERVICE_TAGS): string {
  return SERVICE_TAGS[tag] || tag;
}

/**
 * Create custom NSG rule
 */
export function createNsgRule(config: Partial<NsgRuleConfig> & { name: string }): NsgRuleConfig {
  return {
    name: config.name,
