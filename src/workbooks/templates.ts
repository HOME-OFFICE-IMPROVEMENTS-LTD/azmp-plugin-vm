...
const ADVANCED_MONITORING_TEMPLATES: WorkbookTemplate[] = advancedWorkbookTemplates.map(template => ({
  id: template.id,
  name: template.name,
...
export class WorkbookTemplateManager {
   ...
   */
  public static getTemplatesByCategory(category: WorkbookTemplate['category']): WorkbookTemplate[] {
   ...
   */
  private static applyCustomParameters(workbook: any, parameters: Record<string, any>): void {
   ...
   */
  public static validateTemplate(template: WorkbookTemplate): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
