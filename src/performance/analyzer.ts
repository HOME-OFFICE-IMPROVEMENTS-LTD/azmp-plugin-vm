...
export class PerformanceAnalyzer {
  ...
  static analyzePerformance(
    ...
    if (!specs) {
      throw new Error(
        `VM size ${vmSize} not supported for performance analysis`,
      );
    }
  ...
  static analyzeBurstCapability(
    ...
    return {
      ...
      recommendations: {
        ...
        reasoning: this.getBurstRecommendationReasoning(
          metrics,
          specs,
          sustainabilityMinutes,
        ),
  ...
  static analyzeDiskPerformance(
    ...
    const currentThroughput =
      metrics.disk.readThroughput + metrics.disk.writeThroughput;

    // Recommend optimal disk tier based on performance requirements
    const recommendedTier = this.recommendDiskTier(
      currentIops,
      currentThroughput,
    ...
    return {
      ...
      recommendations: {
        recommendedTier,
        reasoning: this.getDiskTierReasoning(
          currentTier,
          recommendedTier,
          currentIops,
          currentThroughput,
        ),
        performanceGain: this.calculateDiskPerformanceGain(
          currentTier,
          recommendedTier,
        ),
      ...
      throughputAnalysis: {
        ...
        recommended: this.getRecommendedThroughput(
          currentThroughput,
          recommendedTier,
        ),
  ...
  static compareVmPerformance(
    ...
    const results = vmSizes
      .map((vmSize) => {
        ...
        const { pros, cons } = this.getVmSizeProsAndCons(
          specs,
          workloadProfile,
        );
  ...
  private static calculatePerformanceScore(
    ...
    const iopsUtilization =
      ((metrics.disk.readIops + metrics.disk.writeIops) / specs.maxIops) * 100;
  ...
  private static identifyBottlenecks(
    ...
    if (metrics.cpu.average > 85) {
      bottlenecks.push({
        ...
        impact:
          "Application performance degradation and response time increases",
        suggestedActions: [
  ...
  private static generateRecommendations(
    ...
    if (
      utilization.cpuUtilization === "overutilized" ||
    ...
    if (bottlenecks.some((b) => b.resource === "disk")) {
      recommendations.push({
        ...
        description:
          "Current disk performance is limiting application performance",
  ...
  private static findOptimizationOpportunities(
    ...
    if (specs.burstSupported && metrics.cpu.peak > metrics.cpu.average * 2) {
      opportunities.push({
        ...
        description:
          "Workload shows burst patterns suitable for burst-capable VMs",
  ...
  private static getRecommendedThroughput(
    ...
    return Math.min(
      currentThroughput * 1.5,
      tierMaxThroughput[tier] || currentThroughput,
    );
  }
