...
export function registerPerformanceHelpers(): void {
  ...
  Handlebars.registerHelper('perf:analyzeVm', function(vmSize: string, metricsInput: unknown) {
    try {
      const metrics = parseJsonArgument<PerformanceMetrics>('metrics', metricsInput);
      const analysis = PerformanceAnalyzer.analyzePerformance(vmSize, metrics);
  ...
  Handlebars.registerHelper('perf:burstAnalysis', function(vmSize: string, metricsInput: unknown) {
    try {
      const metrics = parseJsonArgument<PerformanceMetrics>('metrics', metricsInput);
      const result = PerformanceAnalyzer.analyzeBurstCapability(vmSize, metrics);
  ...
  Handlebars.registerHelper('perf:diskOptimization', function(currentTier: string, vmSize: string, metricsInput: unknown) {
    try {
      const metrics = parseJsonArgument<PerformanceMetrics>('metrics', metricsInput);
      const result = PerformanceAnalyzer.analyzeDiskPerformance(currentTier, metrics, vmSize);
      return safeStringify(result);
    } catch (error) {
  ...
  Handlebars.registerHelper('perf:compareVmPerformance', function(vmSizes: string | unknown[], workloadProfile: string) {
    try {
      const list = Array.isArray(vmSizes)
        ? vmSizes.map(size => String(size)).filter(Boolean)
        : typeof vmSizes === 'string'
          ? vmSizes.split(',').map((size) => size.trim()).filter(Boolean)
          : [];
      ...
      const profile = workloadProfile as 'cpu-intensive' | 'memory-intensive' | 'io-intensive' | 'balanced';
  ...
  Handlebars.registerHelper('perf:loadPattern', function(metricsInput: unknown) {
    try {
      const metrics = parseJsonArgument<Array<{ timestamp: string | Date; cpuPercent: number; memoryPercent: number }>>(
        'metrics',
        metricsInput
      ).map((entry) => ({
        ...entry,
  ...
  Handlebars.registerHelper('perf:autoscaleConfig', function(resourceUri: string, loadPatternInput: unknown, options: any) {
    try {
      const loadPattern = parseJsonArgument<LoadPattern>('loadPattern', loadPatternInput);
      const config = AutoscaleEngine.generateAutoscaleConfiguration(resourceUri, loadPattern, {
  ...
  Handlebars.registerHelper('perf:predictiveScaling', function(loadPatternInput: unknown, configInput?: unknown) {
    try {
      const loadPattern = parseJsonArgument<LoadPattern>('loadPattern', loadPatternInput);
      const configuration = configInput ? parseJsonArgument<AutoscaleConfiguration>('configuration', configInput) : undefined;
      const recommendation = AutoscaleEngine.generatePredictiveScalingRecommendation(loadPattern, configuration);
  ...
  Handlebars.registerHelper('perf:simulateScaling', function(metricsInput: unknown, configInput: unknown, options: any) {
    try {
      const metrics = parseJsonArgument<Array<{ timestamp: string | Date; cpuPercent: number; instances: number }>>(
        'metrics',
        metricsInput
      ).map((entry) => ({
        ...entry,
      ...
      const configuration = parseJsonArgument<AutoscaleConfiguration>('configuration', configInput);
      const vmSize = options?.hash?.vmSize ?? 'Standard_D2s_v3';

      const simulation = AutoscaleEngine.simulateScaling(metrics, configuration, vmSize);
  ...
  Handlebars.registerHelper('perf:optimizedRules', function(loadPatternInput: unknown, options: any) {
    try {
      const loadPattern = parseJsonArgument<LoadPattern>('loadPattern', loadPatternInput);
      const costOptimized = Boolean(options?.hash?.costOptimized);
      const rules = AutoscaleEngine.generateOptimizedRules(loadPattern, costOptimized);
