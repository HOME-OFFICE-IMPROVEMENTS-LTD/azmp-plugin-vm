...
export class AutoscaleEngine {
  ...
  static generatePredictiveScalingRecommendation(
    ...
    if (shouldEnable && confidence > 70) {
      return {
        ...
        confidence,
        reasoning:
          "Load pattern shows predictable trends suitable for predictive scaling",
        benefits: [
      ...
      currentConfig?.predictiveAutoscalePolicy?.scaleMode === "Enabled"
    ) {
      return {
        recommendedAction: "optimize",
        confidence,
        reasoning:
          "Current predictive scaling can be optimized for better performance",
        benefits: [
      ...
      return {
        ...
        confidence,
        reasoning:
          "Load pattern is too unpredictable for effective predictive scaling",
        benefits: [
    ...
    for (let i = 0; i < metrics.length; i++) {
      ...
      if (scalingDecision.shouldScale) {
        events.push({
          ...
          action:
            scalingDecision.direction === "out" ? "scale-out" : "scale-in",
  ...
  private static analyzePeriodicPatterns(
    metrics: Array<{ timestamp: Date; cpuPercent: number }>,
  ): LoadPattern["periodicityAnalysis"] {
  ...
  private static analyzeHourlyPattern(
    ...
    const overallAverage =
      hourlyAverages.reduce((sum, h) => sum + h.average, 0) / 24;
  ...
  private static analyzeDailyPattern(
    ...
    const overallAverage =
      dailyAverages.reduce((sum, d) => sum + d.average, 0) / 7;
  ...
  private static shouldEnablePredictiveScaling(
    ...
    return (
      loadPattern.patternType === "periodic" &&
  ...
  private static createWeekendProfile(
    ...
    return {
      ...
      capacity: {
        minimum: Math.max(
          1,
          loadPattern.scalingRecommendations.recommendedMinInstances - 1,
        ),
        maximum: Math.ceil(
          loadPattern.scalingRecommendations.recommendedMaxInstances * 0.7,
        ),
        default: Math.max(
          1,
          Math.ceil(loadPattern.characteristics.baselineLoad / 70),
        ),
      },
