...
describe("Performance Optimization Module", () => {
  ...
  describe("PerformanceAnalyzer", () => {
    ...
    it("recommends disk tier adjustments", () => {
      const disk = PerformanceAnalyzer.analyzeDiskPerformance(
        "Standard_SSD",
        sampleMetrics,
        "Standard_D4s_v3",
      );
  ...
  describe("AutoscaleEngine", () => {
    it("detects load patterns from metrics", () => {
      ...
      expect(
        loadPattern.scalingRecommendations.recommendedMaxInstances,
      ).toBeGreaterThan(
    ...
    it("generates autoscale configuration", () => {
      ...
      if (config.notifications) {
        expect(config.notifications[0].email?.customEmails).toContain(
          "ops@example.com",
        );
      }
    });
  ...
  describe("Performance Handlebars helpers", () => {
    it("renders perf:analyzeVm helper output", () => {
      const template = Handlebars.compile(
        `{{perf:analyzeVm "Standard_D4s_v3" metrics}}`,
      );
    ...
    it("renders perf:simulateScaling helper output", () => {
      ...
      const template = Handlebars.compile(
        `{{perf:simulateScaling metrics config}}`,
      );
