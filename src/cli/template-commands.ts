        ...
        for (const file of files) {
          ...
          try {
            ...
            if (fileName === 'mainTemplate.json') {
              if (!json.$schema || !json.$schema.includes('deploymentTemplate.json')) {
              ...
              if (!json.$schema || !json.$schema.includes('createUiDefinition')) {
                context.logger.error('  ✗ Invalid or missing $schema property');
                allValid = false;
                totalErrors++;
              } else {
  ...
  templateCommand
    ...
    .action(async (templatePath: string, cmdOptions) => {
      try {
        ...
        for (const [paramName, paramDef] of Object.entries(templateParams) as [string, any][]) {
