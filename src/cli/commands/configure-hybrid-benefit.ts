...
import {
  ...
  LicenseType,
  HybridBenefitConfig
} from '../../azure/hybrid-benefit';

const configureHybridBenefitCommand = new Command('configure-hybrid-benefit')
    ...
    try {
      ...
      if (options.listSizes) {
        ...
        sizes.forEach((size: string) => {
          const manager = new HybridBenefitManager({
            vmName: 'estimate',
            vmSize: size,
            osType: 'Windows',
      ...
      if (options.estimateOnly) {
        ...
        const sqlServerEdition = options.sqlEdition as 'Enterprise' | 'Standard' | undefined;

        const savings = HybridBenefitManager.estimateSavings(
          options.vmSize,
          options.osType as 'Windows' | 'Linux',
          hasSqlServer,
          sqlServerEdition
        );
      ...
      const config: HybridBenefitConfig = {
        ...
        osType: options.osType as 'Windows' | 'Linux',
        licenseType: LicenseType[options.licenseType.replace('_', '') as keyof typeof LicenseType],
        hasSqlServer: options.sqlServer === true,
        sqlServerEdition: options.sqlEdition as 'Enterprise' | 'Standard' | undefined,
...
export default configureHybridBenefitCommand;
