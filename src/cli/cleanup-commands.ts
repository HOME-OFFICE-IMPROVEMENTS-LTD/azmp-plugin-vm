...
function getCleanupScriptPath(): string {
  ...
  return path.join(
    __dirname,
    "../../scripts/cleanup/Delete-RecoveryServicesVault.ps1",
  );
}
 ...
 */
async function executeVaultCleanup(
  options: VaultCleanupOptions,
  context: PluginContext,
): Promise<void> {
  ...
  context.logger.info(
    `   Mode: ${options.dryRun ? "DRY RUN (preview only)" : "EXECUTION"}`,
  );
  ...
  if (options.force && !options.dryRun && enforceApproval) {
    ...
    const approval = approvalManager.findApprovalByVault(
      options.vaultName!,
    ...
    context.logger.info(
      `   Valid for: ${hoursRemaining}h ${minutesRemaining}m`,
    );
  }
  ...
  if ((options as any).json) {
    ...
    if (result.success) {
      ...
      throw new Error(
        `PowerShell script failed with exit code ${result.exitCode}`,
      );
    }
  } else {
    // Normal mode
...
async function executeVaultApprove(
  ...
  if (dryRunResult.mode !== "dry-run") {
    throw new Error(
      'Can only approve dry-run results (mode must be "dry-run")',
    );
  }
  ...
  context.logger.info(
    `   Resource Group: ${dryRunResult.vaultInfo.resourceGroup}`,
  );
  context.logger.info(
    `   Subscription: ${dryRunResult.vaultInfo.subscription}`,
  );
  context.logger.info(`   Operations: ${dryRunResult.operations.length}`);
  context.logger.info(`   Hash: ${dryRunResult.hash}`);
...
async function executeVaultCheck(
  ...
  if (isExpired) {
    ...
    context.logger.info("");
    context.logger.info(
      "You need to create a new approval before executing --force",
    );
    process.exit(1);
  }
  ...
  context.logger.info(
    `✓ Approval valid for ${hoursRemaining}h ${minutesRemaining}m`,
  );
}
...
export function registerCleanupCommands(
  ...
  cleanupCommand
    ...
    .option(
      "--force",
      "Skip all safety confirmations (DANGEROUS - use with extreme caution)",
    )
  ...
  cleanupCommand
    ...
    .option(
      "--ttl <hours>",
      "Time-to-live for approval in hours (default: 24)",
      "24",
    )
  ...
  cleanupCommand
    ...
    .action(
      async (cmdOptions: { vaultName?: string; resourceGroup?: string }) => {
        try {
