...
export const VM_SIZES: Record<string, VmSize> = {
...
export function getVmSizesByFamily(family: VmSizeFamily): VmSize[] {
  return Object.values(VM_SIZES).filter((size) => size.family === family);
}

export function getVmSize(name: string): VmSize | undefined {
  return VM_SIZES[name];
}

export function getAllVmSizeFamilies(): Array<{ key: VmSizeFamily; name: string }> {
  return Object.entries(VM_SIZE_FAMILIES).map(([key, name]) => ({
    key: key as VmSizeFamily,
    name,
  }));
}

export function searchVmSizes(query: string): VmSize[] {
  const lowerQuery = query.toLowerCase();
  return Object.values(VM_SIZES).filter(
    (size) =>
      size.name.toLowerCase().includes(lowerQuery) ||
