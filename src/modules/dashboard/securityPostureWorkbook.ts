 ...
 */
export function workbookSecurityPosture(this: unknown, options: SecurityPostureWorkbookOptions): string {
  if (!options.name) throw new Error('name is required for workbook:securityPosture');
  if (!options.location) throw new Error('location is required for workbook:securityPosture');
  if (!options.resourceGroup) throw new Error('resourceGroup is required for workbook:securityPosture');
  if (!options.subscriptionId) throw new Error('subscriptionId is required for workbook:securityPosture');
...
export function registerSecurityPostureWorkbookHelpers(): void {
  Handlebars.registerHelper('workbook:securityPosture', function (this: unknown, options: unknown) {
    const opts = (options as { hash?: SecurityPostureWorkbookOptions })?.hash || options;
    return new Handlebars.SafeString(workbookSecurityPosture.call(this, opts as SecurityPostureWorkbookOptions));
  });
}
