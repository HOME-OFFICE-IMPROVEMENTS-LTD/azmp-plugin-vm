 ...
 */
export function dashboardVmssScaling(this: unknown, options: VmssScalingDashboardOptions): string {
  if (!options.name) throw new Error('name is required for dashboard:vmssScaling');
  if (!options.location) throw new Error('location is required for dashboard:vmssScaling');
  if (!options.vmssResourceId) throw new Error('vmssResourceId is required for dashboard:vmssScaling');
          ...
          content: {
            options: {
              chart: {
                ...
                titleKind: 1,
                visualization: { chartType: 3, legendVisualization: { isVisible: true } }, // Line chart
              ...
              chart: {
                metrics: [
                  ...
                  },
                  {
                    ...
                    aggregationType: 4,
                    namespace: 'Microsoft.Compute/virtualMachineScaleSets',
                    metricVisualization: { displayName: 'Outbound Connections' }
                  }
                ],
    ...
    properties: {
      ...
      metadata: {
        model: {
          ...
          filters: { value: { MsPortalFx_TimeRange: { model: { format: 'utc', granularity: 'auto', relative: refreshInterval }, displayCache: {}, filteredPartIds: [] } } }
        }
      }
    }
  };
...
export function registerVmssScalingDashboardHelpers(): void {
  Handlebars.registerHelper('dashboard:vmssScaling', function (this: unknown, options: unknown) {
    const opts = (options as { hash?: VmssScalingDashboardOptions })?.hash || options;
    return new Handlebars.SafeString(dashboardVmssScaling.call(this, opts as VmssScalingDashboardOptions));
  });
}
