 ...
 */
export function dashboardCostAnalysis(
  this: unknown,
  options: CostAnalysisDashboardOptions,
): string {
  if (!options.name)
  ...
  const scope =
    options.resourceGroups && options.resourceGroups.length > 0
...
export function registerCostAnalysisDashboardHelpers(): void {
  Handlebars.registerHelper(
    ...
    function (this: unknown, options: unknown) {
      const opts =
        (options as { hash?: CostAnalysisDashboardOptions })?.hash || options;
      return new Handlebars.SafeString(
        dashboardCostAnalysis.call(this, opts as CostAnalysisDashboardOptions),
      );
    },
  );
}
