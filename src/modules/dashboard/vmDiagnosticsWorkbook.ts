 ...
 */
export function workbookVmDiagnostics(this: unknown, options: VmDiagnosticsWorkbookOptions): string {
  if (!options.name) throw new Error('name is required for workbook:vmDiagnostics');
  if (!options.location) throw new Error('location is required for workbook:vmDiagnostics');
  if (!options.resourceGroup) throw new Error('resourceGroup is required for workbook:vmDiagnostics');
  if (!options.vmResourceIds || options.vmResourceIds.length === 0) {
    throw new Error('vmResourceIds array is required for workbook:vmDiagnostics');
  }
...
export function registerVmDiagnosticsWorkbookHelpers(): void {
  Handlebars.registerHelper('workbook:vmDiagnostics', function (this: unknown, options: unknown) {
    const opts = (options as { hash?: VmDiagnosticsWorkbookOptions })?.hash || options;
    return new Handlebars.SafeString(workbookVmDiagnostics.call(this, opts as VmDiagnosticsWorkbookOptions));
  });
}
