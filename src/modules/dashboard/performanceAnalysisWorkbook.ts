 ...
 */
export function workbookPerformanceAnalysis(this: unknown, options: PerformanceAnalysisWorkbookOptions): string {
  if (!options.name) throw new Error('name is required for workbook:performanceAnalysis');
  if (!options.location) throw new Error('location is required for workbook:performanceAnalysis');
  if (!options.resourceGroup) throw new Error('resourceGroup is required for workbook:performanceAnalysis');
  if (!options.workspaceResourceId) throw new Error('workspaceResourceId is required for workbook:performanceAnalysis');
...
export function registerPerformanceAnalysisWorkbookHelpers(): void {
  Handlebars.registerHelper('workbook:performanceAnalysis', function (this: unknown, options: unknown) {
    const opts = (options as { hash?: PerformanceAnalysisWorkbookOptions })?.hash || options;
    return new Handlebars.SafeString(workbookPerformanceAnalysis.call(this, opts as PerformanceAnalysisWorkbookOptions));
  });
}
